...
public class LogController : ControllerBase
{
    ...
    public LogController(IConfiguration configuration, WBMDatabase wBMDB)
    {
    ...
    {
        string APIEndPointName = "LogSearch";

        wbm_common.DataObjects.Log.dbRow apiTransactionLog = new wbm_common.DataObjects.Log.dbRow();

        try
        {
            APILoggingReturn APICommonReturn = await CommonAPILogging(User, Request, APIEndPointName, _WBMDB, apiTransactionLog, "", "", new wbm_common.DataObjects.Log.Search());
            ...
            Result<List<wbm_common.DataObjects.Log.dbRow>> LogSearchResult = await LogHelper.ExecuteSearch((wbm_common.DataObjects.Log.Search)APICommonReturn.genericObject, _WBMDB);

            if (LogSearchResult.IsException)
                return Helpers.ActionResultUtils.Send500Response(new List<ErrorMessage> { new ErrorMessage { Error = LogSearchResult.ErrorMessage } }, false, apiTransactionLog, _WBMDB);
            else if (LogSearchResult.IsFailure)
                return Helpers.ActionResultUtils.Send400Response(new List<ErrorMessage> { new ErrorMessage { Error = LogSearchResult.ErrorMessage } }, false, apiTransactionLog, _WBMDB);
            else
                return Helpers.ActionResultUtils.Send200Response(LogSearchResult.Value, apiTransactionLog, _WBMDB);
        }
        catch (Exception ex)
        {
    ...
    [HttpPost]
    [Route("api/LogExcelReport")]
    public async Task<IActionResult> ExcelReport()
    {
        string APIEndPointName = "LogExcelReport";
        ...
        try
        {
            ...
            LogExcel CE = new LogExcel((wbm_common.DataObjects.Log.Search)APICommonReturn.genericObject, _WBMDB);

            if (await CE.GenerateExcel())
            {
#if DEBUG
                return await debugWriteFile(CE.ms, apiTransactionLog, _WBMDB, "LogExcelReport");
#else
