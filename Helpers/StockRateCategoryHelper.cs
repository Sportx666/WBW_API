...
{
    public class StockRateCategoryHelper
    {
        private static Logger logger = LogManager.Setup().LoadConfigurationFromAppSettings().GetCurrentClassLogger();
        public static async Task<Result<List<wbm_common.DataObjects.StockRateCategory.dbRow>>> ExecuteSearch(wbm_common.DataObjects.StockRateCategory.Search _StockRateCategorySearchData, WBMDatabase _WBMDB, IHubCallerClients Clients = null, Func<IHubCallerClients, WBMDatabase, string, int, int, wbm_common.DataObjects.LongRunProcessHeader.dbRow, Task<string>> inCallBackStatusFunction = null, wbm_common.DataObjects.LongRunProcessHeader.dbRow longRunProcessHeader = null)
        {
            Result<List<wbm_common.DataObjects.StockRateCategory.dbRow>> rc;
            List<wbm_common.DataObjects.StockRateCategory.dbRow> StockRateCategorys;
            switch (_StockRateCategorySearchData.SearchMode)
            {
                case wbm_common.DataObjects.StockRateCategory.Search.SearchModeType.Undefined:
                    {
                        _StockRateCategorySearchData.GenerateSqlQuery();
                        rc = await _WBMDB.StockRateCategory_ListBySql(_StockRateCategorySearchData.SqlQuery, _StockRateCategorySearchData.ListSqlParameter);
                        break;
                    }
                case wbm_common.DataObjects.StockRateCategory.Search.SearchModeType.SingleID:
                    {
                        rc = await _WBMDB.StockRateCategory_SearchByID(_StockRateCategorySearchData.ID);
                        break;
                    }
                case wbm_common.DataObjects.StockRateCategory.Search.SearchModeType.AllRecords:
                    {
                        rc = await _WBMDB.StockRateCategory_List();
                        break;
                    }
                case wbm_common.DataObjects.StockRateCategory.Search.SearchModeType.SingleSite:
                    {
                        if (_StockRateCategorySearchData.ListSiteID == null || _StockRateCategorySearchData.ListSiteID.Count() != 1)
                        {
                            logger.Error("StockRateCategoryHelper.ExecuteSearch: Stock SingleSite doesn't have single site input");
                            rc = Result.Exception<List<wbm_common.DataObjects.StockRateCategory.dbRow>>("Internal Exception occured");
                            break;
                        }
                        rc = await _WBMDB.StockRateCategory_ListBySingleSiteID(_StockRateCategorySearchData.ListSiteID[0]);
                        break;
                    }
                case wbm_common.DataObjects.StockRateCategory.Search.SearchModeType.SingleCompany:
                    {
                        if (_StockRateCategorySearchData.ListCompanyID == null || _StockRateCategorySearchData.ListCompanyID.Count() != 1)
                        {
                            logger.Error("StockRateCategoryHelper.ExecuteSearch: Stock SingleCompany doesn't have single company input");
                            rc = Result.Exception<List<wbm_common.DataObjects.StockRateCategory.dbRow>>("Internal Exception occured");
                            break;
                        }
                        rc = await _WBMDB.StockRateCategory_ListBySingleCompanyID(_StockRateCategorySearchData.ListCompanyID[0]);
                        break;
                    }
                default:
                    {
                        logger.Error("StockRateCategoryHelper.ExecuteSearch: StockRateCategory Search Mode Type Error");
                        rc = Result.Exception<List<wbm_common.DataObjects.StockRateCategory.dbRow>>("Internal Exception occured");
                        break;
                    }
            }
        ...
        public class StockRateCategorySave
        {
            private wbm_common.DataObjects.StockRateCategory saveObject { get; set; }
            private wbm_common.DataObjects.StockRateCategory.dbRow saveRow { get; set; }

            private wbm_common.DataObjects.StockRateCategory.dbRow existRow { get; set; }
            ...
            public StockRateCategorySave(wbm_common.DataObjects.StockRateCategory saveObject, int UserID, WBMDatabase wBMDB)
            {
            ...
            public Boolean CheckExist()
            {
                ...
                if (saveRow.ID > 0)
                {
                    Result<wbm_common.DataObjects.StockRateCategory.dbRow> rc = _WBMDB.StockRateCategory_ReadByID(saveRow.ID).GetAwaiter().GetResult();
                ...
                {
                    Result<wbm_common.DataObjects.StockRateCategory.dbRow> rcexist = _WBMDB.StockRateCategory_SearchByPublicID(saveRow.PublicID).GetAwaiter().GetResult();
                    ...
                    else
                    {
                        ...
                        logger.Error("StockRateCategorySave ID " + saveRow.ID.ToString() + " PublicID " + saveRow.PublicID + " already exists");
                        ReturnState = 400;
                        SavedSuccessfully = false;
                        return false;
                    }
            ...
            public Boolean CheckForChanges()
            {
                ...
                return ct.CheckForChanges(existRow, saveRow, wbm_common.DataObjects.StockRateCategory.ListChangeTrackerData);
            }
            ...
            public void SetChanged()
            {
                ...
                else
                {
                    ...
                    changeHeaderRow = WBM_API.Helpers.ChangeTracking.Create_ChangeHeader_dbRow((int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.StockRateCategory,
            ...
            {
                Result<int> sr = _WBMDB.StockRateCategory_Insert(saveRow).GetAwaiter().GetResult();
            ...
            public Boolean Save_Update_or_Delete()
            {
                ...
                Result<int> cuResult = _WBMDB.StockRateCategory_Update(saveRow).GetAwaiter().GetResult();
                ...
                if (cuResult.IsSuccess)
                {
                    ...
                    if (chResult.IsSuccess)
                    {
                        ...
                        int ChangeHeaderTableID = (int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.StockRateCategory;
