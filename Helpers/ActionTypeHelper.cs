...
namespace WBM_API.Helpers
{
    public class ActionTypeHelper
    {
        ...
        public static async Task<Result<List<wbm_common.DataObjects.ActionType.dbRow>>> ExecuteSearch(wbm_common.DataObjects.ActionType.Search _ActionTypeSearchData, WBMDatabase _WBMDB, IHubCallerClients Clients = null, Func<IHubCallerClients, WBMDatabase, string, int, int, wbm_common.DataObjects.LongRunProcessHeader.dbRow, Task<string>> inCallBackStatusFunction = null, wbm_common.DataObjects.LongRunProcessHeader.dbRow longRunProcessHeader = null)
        {
            Result<List<wbm_common.DataObjects.ActionType.dbRow>> rc;
            List<wbm_common.DataObjects.ActionType.dbRow> ActionTypes;
            switch (_ActionTypeSearchData.SearchMode)
            {
                case wbm_common.DataObjects.ActionType.Search.SearchModeType.Undefined:
                    {
                        _ActionTypeSearchData.GenerateSqlQuery();
                        rc = await _WBMDB.ActionType_ListBySql(_ActionTypeSearchData.SqlQuery, _ActionTypeSearchData.ListSqlParameter);
                        break;
                    }
                case wbm_common.DataObjects.ActionType.Search.SearchModeType.SingleID:
                    {
                        rc = await _WBMDB.ActionType_SearchByID(_ActionTypeSearchData.ID);
                        break;
                    }
                case wbm_common.DataObjects.ActionType.Search.SearchModeType.AllRecords:
                    {
                        rc = await _WBMDB.ActionType_List();
                        break;
                    }
                default:
                    {
                        logger.Error("ActionTypeHelper.ExecuteSearch: ActionType Search Mode Type Error");
                        rc = Result.Exception<List<wbm_common.DataObjects.ActionType.dbRow>>("Internal Exception occured");
                        break;
                    }
            }
        ...
        public class ActionTypeSave
        {
            private wbm_common.DataObjects.ActionType saveObject { get; set; }
            private wbm_common.DataObjects.ActionType.dbRow saveRow { get; set; }

            private wbm_common.DataObjects.ActionType.dbRow existRow { get; set; }
            ...
            public ActionTypeSave(wbm_common.DataObjects.ActionType saveObject, int UserID, WBMDatabase wBMDB)
            {
            ...
            public Boolean CheckExist()
            {
                ...
                if (saveRow.ID > 0)
                {
                    Result<wbm_common.DataObjects.ActionType.dbRow> rc = _WBMDB.ActionType_ReadByID(saveRow.ID).GetAwaiter().GetResult();
                ...
                {
                    Result<wbm_common.DataObjects.ActionType.dbRow> rcexist = _WBMDB.ActionType_SearchByPublicID(saveRow.PublicID).GetAwaiter().GetResult();
                    ...
                    else
                    {
                        ...
                        logger.Error("ActionTypeSave ID " + saveRow.ID.ToString() + " PublicID " + saveRow.PublicID + " already exists");
                        ReturnState = 400;
                        SavedSuccessfully = false;
                        return false;
                    }
            ...
            public Boolean CheckForChanges()
            {
                ...
                return ct.CheckForChanges(existRow, saveRow, wbm_common.DataObjects.ActionType.ListChangeTrackerData);
            }
            ...
            public void SetChanged()
            {
                ...
                else
                {
                    ...
                    changeHeaderRow = WBM_API.Helpers.ChangeTracking.Create_ChangeHeader_dbRow((int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.ActionType,
            ...
            {
                Result<int> sr = _WBMDB.ActionType_Insert(saveRow).GetAwaiter().GetResult();
            ...
            public Boolean Save_Update_or_Delete()
            {
                ...
                Result<int> cuResult = _WBMDB.ActionType_Update(saveRow).GetAwaiter().GetResult();
                ...
                if (cuResult.IsSuccess)
                {
                    ...
                    if (chResult.IsSuccess)
                    {
                        ...
                        int ChangeHeaderTableID = (int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.ActionType;
