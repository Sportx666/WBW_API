...
{
    public class TransactionSourceHelper
    {
        private static Logger logger = LogManager.Setup().LoadConfigurationFromAppSettings().GetCurrentClassLogger();
        public static async Task<Result<List<wbm_common.DataObjects.TransactionSource.dbRow>>> ExecuteSearch(wbm_common.DataObjects.TransactionSource.Search _TransactionSourceSearchData, WBMDatabase _WBMDB, IHubCallerClients Clients = null, Func<IHubCallerClients, WBMDatabase, string, int, int, wbm_common.DataObjects.LongRunProcessHeader.dbRow, Task<string>> inCallBackStatusFunction = null, wbm_common.DataObjects.LongRunProcessHeader.dbRow longRunProcessHeader = null)
        {
            Result<List<wbm_common.DataObjects.TransactionSource.dbRow>> rc;
            List<wbm_common.DataObjects.TransactionSource.dbRow> TransactionSources;
            switch (_TransactionSourceSearchData.SearchMode)
            {
                case wbm_common.DataObjects.TransactionSource.Search.SearchModeType.Undefined:
                    {
                        _TransactionSourceSearchData.GenerateSqlQuery();
                        rc = await _WBMDB.TransactionSource_ListBySql(_TransactionSourceSearchData.SqlQuery, _TransactionSourceSearchData.ListSqlParameter);
                        break;
                    }
                case wbm_common.DataObjects.TransactionSource.Search.SearchModeType.SingleID:
                    {
                        rc = await _WBMDB.TransactionSourceList_SearchByID(_TransactionSourceSearchData.ID);
                        break;
                    }
                case wbm_common.DataObjects.TransactionSource.Search.SearchModeType.AllRecords:
                    {
                        rc = await _WBMDB.TransactionSource_List();
                        break;
                    }
                default:
                    {
                        logger.Error("TransactionSourceHelper.ExecuteSearch: TransactionSource Search Mode Type Error");
                        rc = Result.Exception<List<wbm_common.DataObjects.TransactionSource.dbRow>>("Internal Exception occured");
                        break;
                    }
            }
        ...
        public class TransactionSourceSave
        {
            private wbm_common.DataObjects.TransactionSource saveObject { get; set; }
            private wbm_common.DataObjects.TransactionSource.dbRow saveRow { get; set; }

            private wbm_common.DataObjects.TransactionSource.dbRow existRow { get; set; }
            ...
            public TransactionSourceSave(wbm_common.DataObjects.TransactionSource saveObject, int UserID, WBMDatabase wBMDB)
            {
            ...
            public Boolean CheckExist()
            {
                ...
                if (saveRow.ID > 0)
                {
                    Result<wbm_common.DataObjects.TransactionSource.dbRow> rc = _WBMDB.TransactionSource_ReadByID(saveRow.ID).GetAwaiter().GetResult();
                ...
                {
                    Result<wbm_common.DataObjects.TransactionSource.dbRow> rcexist = _WBMDB.TransactionSource_SearchByID(saveRow.ID).GetAwaiter().GetResult();
                    ...
                    else
                    {
                        ...
                        logger.Error("TransactionSourceSave ID " + saveRow.ID.ToString() + " already exists");
                        ReturnState = 400;
                        SavedSuccessfully = false;
                        return false;
                    }
            ...
            public Boolean CheckForChanges()
            {
                ...
                return ct.CheckForChanges(existRow, saveRow, wbm_common.DataObjects.TransactionSource.ListChangeTrackerData);
            }
            ...
            public void SetChanged()
            {
                ...
                else
                {
                    ...
                    changeHeaderRow = WBM_API.Helpers.ChangeTracking.Create_ChangeHeader_dbRow((int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.TransactionSource,
            ...
            {
                Result<int> sr = _WBMDB.TransactionSource_Insert(saveRow).GetAwaiter().GetResult();
            ...
            public Boolean Save_Update_or_Delete()
            {
                ...
                Result<int> cuResult = _WBMDB.TransactionSource_Update(saveRow).GetAwaiter().GetResult();
                ...
                if (cuResult.IsSuccess)
                {
                    ...
                    if (chResult.IsSuccess)
                    {
                        ...
                        int ChangeHeaderTableID = (int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.TransactionSource;
