...
{
    public class RateCodeHelper
    {
        private static Logger logger = LogManager.Setup().LoadConfigurationFromAppSettings().GetCurrentClassLogger();
        public static async Task<Result<List<wbm_common.DataObjects.RateCode.dbRow>>> ExecuteSearch(wbm_common.DataObjects.RateCode.Search _RateCodeSearchData, WBMDatabase _WBMDB, IHubCallerClients Clients = null, Func<IHubCallerClients, WBMDatabase, string, int, int, wbm_common.DataObjects.LongRunProcessHeader.dbRow, Task<string>> inCallBackStatusFunction = null, wbm_common.DataObjects.LongRunProcessHeader.dbRow longRunProcessHeader = null)
        {
            Result<List<wbm_common.DataObjects.RateCode.dbRow>> rc;
            List<wbm_common.DataObjects.RateCode.dbRow> RateCodes;
            switch (_RateCodeSearchData.SearchMode)
            {
                case wbm_common.DataObjects.RateCode.Search.SearchModeType.Undefined:
                    {
                        _RateCodeSearchData.GenerateSqlQuery();
                        rc = await _WBMDB.RateCode_ListBySql(_RateCodeSearchData.SqlQuery, _RateCodeSearchData.ListSqlParameter);
                        break;
                    }
                case wbm_common.DataObjects.RateCode.Search.SearchModeType.SingleID:
                    {
                        rc = await _WBMDB.RateCodeList_SearchByID(_RateCodeSearchData.ID);
                        break;
                    }
                case wbm_common.DataObjects.RateCode.Search.SearchModeType.AllRecords:
                    {
                        rc = await _WBMDB.RateCode_List();
                        break;
                    }
                default:
                    {
                        logger.Error("RateCodeHelper.ExecuteSearch: RateCode Search Mode Type Error");
                        rc = Result.Exception<List<wbm_common.DataObjects.RateCode.dbRow>>("Internal Exception occured");
                        break;
                    }
            }
        ...
        public class RateCodeSave
        {
            private wbm_common.DataObjects.RateCode saveObject { get; set; }
            private wbm_common.DataObjects.RateCode.dbRow saveRow { get; set; }

            private wbm_common.DataObjects.RateCode.dbRow existRow { get; set; }
            ...
            public RateCodeSave(wbm_common.DataObjects.RateCode saveObject, int UserID, WBMDatabase wBMDB)
            {
            ...
            public Boolean CheckExist()
            {
                ...
                if (saveRow.ID > 0)
                {
                    Result<wbm_common.DataObjects.RateCode.dbRow> rc = _WBMDB.RateCode_ReadByID(saveRow.ID).GetAwaiter().GetResult();
                ...
                {
                    Result<wbm_common.DataObjects.RateCode.dbRow> rcexist = _WBMDB.RateCode_SearchByID(saveRow.ID).GetAwaiter().GetResult();
                    ...
                    else
                    {
                        ...
                        logger.Error("RateCodeSave ID " + saveRow.ID.ToString() + " already exists");
                        ReturnState = 400;
                        SavedSuccessfully = false;
                        return false;
                    }
            ...
            public Boolean CheckForChanges()
            {
                ...
                return ct.CheckForChanges(existRow, saveRow, wbm_common.DataObjects.RateCode.ListChangeTrackerData);
            }
            ...
            public void SetChanged()
            {
                ...
                else
                {
                    ...
                    changeHeaderRow = WBM_API.Helpers.ChangeTracking.Create_ChangeHeader_dbRow((int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.RateCode,
            ...
            {
                Result<int> sr = _WBMDB.RateCode_Insert(saveRow).GetAwaiter().GetResult();
            ...
            public Boolean Save_Update_or_Delete()
            {
                ...
                Result<int> cuResult = _WBMDB.RateCode_Update(saveRow).GetAwaiter().GetResult();
                ...
                if (cuResult.IsSuccess)
                {
                    ...
                    if (chResult.IsSuccess)
                    {
                        ...
                        int ChangeHeaderTableID = (int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.RateCode;
