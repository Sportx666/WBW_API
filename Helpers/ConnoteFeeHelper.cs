...
{
    public class ConnoteFeeHelper
    {
        private static Logger logger = LogManager.Setup().LoadConfigurationFromAppSettings().GetCurrentClassLogger();
        public static async Task<Result<List<wbm_common.DataObjects.ConnoteFee.dbRow>>> ExecuteSearch(wbm_common.DataObjects.ConnoteFee.Search _ConnoteFeeSearchData, WBMDatabase _WBMDB, IHubCallerClients Clients = null, Func<IHubCallerClients, WBMDatabase, string, int, int, wbm_common.DataObjects.LongRunProcessHeader.dbRow, Task<string>> inCallBackStatusFunction = null, wbm_common.DataObjects.LongRunProcessHeader.dbRow longRunProcessHeader = null)
        {
            Result<List<wbm_common.DataObjects.ConnoteFee.dbRow>> rc;
            List<wbm_common.DataObjects.ConnoteFee.dbRow> ConnoteFees;
            switch (_ConnoteFeeSearchData.SearchMode)
            {
                case wbm_common.DataObjects.ConnoteFee.Search.SearchModeType.Undefined:
                    {
                        _ConnoteFeeSearchData.GenerateSqlQuery();
                        rc = await _WBMDB.ConnoteFee_ListBySql(_ConnoteFeeSearchData.SqlQuery, _ConnoteFeeSearchData.ListSqlParameter);
                        break;
                    }
                case wbm_common.DataObjects.ConnoteFee.Search.SearchModeType.SingleID:
                    {
                        rc = await _WBMDB.ConnoteFeeList_SearchByID(_ConnoteFeeSearchData.ID);
                        break;
                    }
                case wbm_common.DataObjects.ConnoteFee.Search.SearchModeType.AllRecords:
                    {
                        rc = await _WBMDB.ConnoteFee_List();
                        break;
                    }
                case wbm_common.DataObjects.ConnoteFee.Search.SearchModeType.SingleOwnerID:
                    {
                        if (_ConnoteFeeSearchData.ListOwnerID == null || _ConnoteFeeSearchData.ListOwnerID.Count() != 1)
                        {
                            logger.Error("ConnoteFeeHelper.ExecuteSearch: ConnoteFee SingleOwner doesn't have single owner input");
                            rc = Result.Exception<List<wbm_common.DataObjects.ConnoteFee.dbRow>>("Internal Exception occured");
                            break;
                        }
                        rc = await _WBMDB.ConnoteFee_ListBySingleOwnerID(_ConnoteFeeSearchData.ListOwnerID[0]);
                        break;
                    }
                default:
                    {
                        logger.Error("ConnoteFeeHelper.ExecuteSearch: ConnoteFee Search Mode Type Error");
                        rc = Result.Exception<List<wbm_common.DataObjects.ConnoteFee.dbRow>>("Internal Exception occured");
                        break;
                    }
            }
        ...
        public class ConnoteFeeSave
        {
            private wbm_common.DataObjects.ConnoteFee saveObject { get; set; }
            private wbm_common.DataObjects.ConnoteFee.dbRow saveRow { get; set; }

            private wbm_common.DataObjects.ConnoteFee.dbRow existRow { get; set; }
            ...
            public ConnoteFeeSave(wbm_common.DataObjects.ConnoteFee saveObject, int UserID, WBMDatabase wBMDB)
            {
            ...
            public Boolean CheckExist()
            {
                ...
                if (saveRow.ID > 0)
                {
                    Result<wbm_common.DataObjects.ConnoteFee.dbRow> rc = _WBMDB.ConnoteFee_ReadByID(saveRow.ID).GetAwaiter().GetResult();
                ...
                {
                    Result<wbm_common.DataObjects.ConnoteFee.dbRow> rcexist = _WBMDB.ConnoteFee_SearchByID(saveRow.ID).GetAwaiter().GetResult();
                    ...
                    else
                    {
                        ...
                        logger.Error("ConnoteFeeSave ID " + saveRow.ID.ToString() + " already exists");
                        ReturnState = 400;
                        SavedSuccessfully = false;
                        return false;
                    }
            ...
            public Boolean CheckForChanges()
            {
                ...
                return ct.CheckForChanges(existRow, saveRow, wbm_common.DataObjects.ConnoteFee.ListChangeTrackerData);
            }
            ...
            public void SetChanged()
            {
                ...
                else
                {
                    ...
                    changeHeaderRow = WBM_API.Helpers.ChangeTracking.Create_ChangeHeader_dbRow((int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.ConnoteFee,
            ...
            {
                Result<int> sr = _WBMDB.ConnoteFee_Insert(saveRow).GetAwaiter().GetResult();
            ...
            public Boolean Save_Update_or_Delete()
            {
                ...
                Result<int> cuResult = _WBMDB.ConnoteFee_Update(saveRow).GetAwaiter().GetResult();
                ...
                if (cuResult.IsSuccess)
                {
                    ...
                    if (chResult.IsSuccess)
                    {
                        ...
                        int ChangeHeaderTableID = (int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.ConnoteFee;
