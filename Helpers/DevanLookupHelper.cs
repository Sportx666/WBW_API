...
{
    public class DevanLookupHelper
    {
        private static Logger logger = LogManager.Setup().LoadConfigurationFromAppSettings().GetCurrentClassLogger();
        public static async Task<Result<List<wbm_common.DataObjects.DevanLookup.dbRow>>> ExecuteSearch(wbm_common.DataObjects.DevanLookup.Search _DevanLookupSearchData, WBMDatabase _WBMDB, IHubCallerClients Clients = null, Func<IHubCallerClients, WBMDatabase, string, int, int, wbm_common.DataObjects.LongRunProcessHeader.dbRow, Task<string>> inCallBackStatusFunction = null, wbm_common.DataObjects.LongRunProcessHeader.dbRow longRunProcessHeader = null)
        {
            Result<List<wbm_common.DataObjects.DevanLookup.dbRow>> rc;
            List<wbm_common.DataObjects.DevanLookup.dbRow> DevanLookups;
            switch (_DevanLookupSearchData.SearchMode)
            {
                case wbm_common.DataObjects.DevanLookup.Search.SearchModeType.Undefined:
                    {
                        _DevanLookupSearchData.GenerateSqlQuery();
                        rc = await _WBMDB.DevanLookup_ListBySql(_DevanLookupSearchData.SqlQuery, _DevanLookupSearchData.ListSqlParameter);
                        break;
                    }
                case wbm_common.DataObjects.DevanLookup.Search.SearchModeType.SingleID:
                    {
                        rc = await _WBMDB.DevanLookupList_SearchByID(_DevanLookupSearchData.ID);
                        break;
                    }
                case wbm_common.DataObjects.DevanLookup.Search.SearchModeType.AllRecords:
                    {
                        rc = await _WBMDB.DevanLookup_List();
                        break;
                    }
                default:
                    {
                        logger.Error("DevanLookupHelper.ExecuteSearch: DevanLookup Search Mode Type Error");
                        rc = Result.Exception<List<wbm_common.DataObjects.DevanLookup.dbRow>>("Internal Exception occured");
                        break;
                    }
            }
        ...
        public class DevanLookupSave
        {
            private wbm_common.DataObjects.DevanLookup saveObject { get; set; }
            private wbm_common.DataObjects.DevanLookup.dbRow saveRow { get; set; }

            private wbm_common.DataObjects.DevanLookup.dbRow existRow { get; set; }
            ...
            public DevanLookupSave(wbm_common.DataObjects.DevanLookup saveObject, int UserID, WBMDatabase wBMDB)
            {
            ...
            public Boolean CheckExist()
            {
                ...
                if (saveRow.ID > 0)
                {
                    Result<wbm_common.DataObjects.DevanLookup.dbRow> rc = _WBMDB.DevanLookup_ReadByID(saveRow.ID).GetAwaiter().GetResult();
                ...
                {
                    Result<wbm_common.DataObjects.DevanLookup.dbRow> rcexist = _WBMDB.DevanLookup_SearchByID(saveRow.ID).GetAwaiter().GetResult();
                    ...
                    else
                    {
                        ...
                        logger.Error("DevanLookupSave ID " + saveRow.ID.ToString() + " already exists");
                        ReturnState = 400;
                        SavedSuccessfully = false;
                        return false;
                    }
            ...
            public Boolean CheckForChanges()
            {
                ...
                return ct.CheckForChanges(existRow, saveRow, wbm_common.DataObjects.DevanLookup.ListChangeTrackerData);
            }
            ...
            public void SetChanged()
            {
                ...
                else
                {
                    ...
                    changeHeaderRow = WBM_API.Helpers.ChangeTracking.Create_ChangeHeader_dbRow((int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.DevanLookup,
            ...
            {
                Result<int> sr = _WBMDB.DevanLookup_Insert(saveRow).GetAwaiter().GetResult();
            ...
            public Boolean Save_Update_or_Delete()
            {
                ...
                Result<int> cuResult = _WBMDB.DevanLookup_Update(saveRow).GetAwaiter().GetResult();
                ...
                if (cuResult.IsSuccess)
                {
                    ...
                    if (chResult.IsSuccess)
                    {
                        ...
                        int ChangeHeaderTableID = (int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.DevanLookup;
