...
namespace WBM_API.Helpers
{
    public class CompanyHelper
    {
        ...
        public static async Task<Result<List<wbm_common.DataObjects.Company.dbRow>>> ExecuteSearch(wbm_common.DataObjects.Company.Search _CompanySearchData, WBMDatabase _WBMDB, IHubCallerClients Clients = null, Func<IHubCallerClients, WBMDatabase, string, int, int, wbm_common.DataObjects.LongRunProcessHeader.dbRow, Task<string>> inCallBackStatusFunction = null, wbm_common.DataObjects.LongRunProcessHeader.dbRow longRunProcessHeader = null)
        {
            Result<List<wbm_common.DataObjects.Company.dbRow>> rc;
            List<wbm_common.DataObjects.Company.dbRow> Companys;
            switch (_CompanySearchData.SearchMode)
            {
                case wbm_common.DataObjects.Company.Search.SearchModeType.Undefined:
                    {
                        _CompanySearchData.GenerateSqlQuery();
                        rc = await _WBMDB.Company_ListBySql(_CompanySearchData.SqlQuery, _CompanySearchData.ListSqlParameter);
                        break;
                    }
                case wbm_common.DataObjects.Company.Search.SearchModeType.SingleID:
                    {
                        rc = await _WBMDB.CompanyList_SearchByID(_CompanySearchData.ID);
                        break;
                    }
                case wbm_common.DataObjects.Company.Search.SearchModeType.AllRecords:
                    {
                        rc = await _WBMDB.Company_List();
                        break;
                    }
                default:
                    {
                        logger.Error("CompanyHelper.ExecuteSearch: Company Search Mode Type Error");
                        rc = Result.Exception<List<wbm_common.DataObjects.Company.dbRow>>("Internal Exception occured");
                        break;
                    }
            }
        ...
        {
            private wbm_common.DataObjects.Company saveObject { get; set; }
            private wbm_common.DataObjects.Company.dbRow saveRow { get; set; }

            private wbm_common.DataObjects.Company.dbRow existRow { get; set; }
            ...
            public CompanySave(wbm_common.DataObjects.Company saveObject, int UserID, WBMDatabase wBMDB)
            {
            ...
            public Boolean CheckExist()
            {
                ...
                if (saveRow.ID > 0)
                {
                    Result<wbm_common.DataObjects.Company.dbRow> rc = _WBMDB.Company_ReadByID(saveRow.ID).GetAwaiter().GetResult();
                ...
                {
                    Result<wbm_common.DataObjects.Company.dbRow> rcexist = _WBMDB.Company_SearchByID(saveRow.ID).GetAwaiter().GetResult();
                    ...
                    else
                    {
                        ...
                        ErrorMessage = "PublicID already in use, must be unique - " + saveRow.ID.ToString();
                        logger.Error("CompanySave ID " + saveRow.ID.ToString() + " already exists");
                        ReturnState = 400;
                        SavedSuccessfully = false;
                        return false;
                    }
            ...
            public Boolean CheckForChanges()
            {
                ...
                return ct.CheckForChanges(existRow, saveRow, wbm_common.DataObjects.Company.ListChangeTrackerData);
            }
            ...
            public void SetChanged()
            {
                ...
                else
                {
                    ...
                    changeHeaderRow = WBM_API.Helpers.ChangeTracking.Create_ChangeHeader_dbRow((int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.Company,
            ...
            {
                Result<int> sr = _WBMDB.Company_Insert(saveRow).GetAwaiter().GetResult();
            ...
            public Boolean Save_Update_or_Delete()
            {
                ...
                Result<int> cuResult = _WBMDB.Company_Update(saveRow).GetAwaiter().GetResult();
                ...
                if (cuResult.IsSuccess)
                {
                    ...
                    if (chResult.IsSuccess)
                    {
                        ...
                        int ChangeHeaderTableID = (int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.Company;
