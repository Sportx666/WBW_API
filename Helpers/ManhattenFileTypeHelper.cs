...
{
    public class ManhattenFileTypeHelper
    {
        private static Logger logger = LogManager.Setup().LoadConfigurationFromAppSettings().GetCurrentClassLogger();
        public static async Task<Result<List<wbm_common.DataObjects.ManhattenFileType.dbRow>>> ExecuteSearch(wbm_common.DataObjects.ManhattenFileType.Search _ManhattenFileTypeSearchData, WBMDatabase _WBMDB, IHubCallerClients Clients = null, Func<IHubCallerClients, WBMDatabase, string, int, int, wbm_common.DataObjects.LongRunProcessHeader.dbRow, Task<string>> inCallBackStatusFunction = null, wbm_common.DataObjects.LongRunProcessHeader.dbRow longRunProcessHeader = null)
        {
            Result<List<wbm_common.DataObjects.ManhattenFileType.dbRow>> rc;
            List<wbm_common.DataObjects.ManhattenFileType.dbRow> ManhattenFileTypes;
            switch (_ManhattenFileTypeSearchData.SearchMode)
            {
                case wbm_common.DataObjects.ManhattenFileType.Search.SearchModeType.Undefined:
                    {
                        _ManhattenFileTypeSearchData.GenerateSqlQuery();
                        rc = await _WBMDB.ManhattenFileType_ListBySql(_ManhattenFileTypeSearchData.SqlQuery, _ManhattenFileTypeSearchData.ListSqlParameter);
                        break;
                    }
                case wbm_common.DataObjects.ManhattenFileType.Search.SearchModeType.SingleID:
                    {
                        rc = await _WBMDB.ManhattenFileTypeList_SearchByID(_ManhattenFileTypeSearchData.ID);
                        break;
                    }
                case wbm_common.DataObjects.ManhattenFileType.Search.SearchModeType.AllRecords:
                    {
                        rc = await _WBMDB.ManhattenFileType_List();
                        break;
                    }
                default:
                    {
                        logger.Error("ManhattenFileTypeHelper.ExecuteSearch: ManhattenFileType Search Mode Type Error");
                        rc = Result.Exception<List<wbm_common.DataObjects.ManhattenFileType.dbRow>>("Internal Exception occured");
                        break;
                    }
            }
        ...
        public class ManhattenFileTypeSave
        {
            private wbm_common.DataObjects.ManhattenFileType saveObject { get; set; }
            private wbm_common.DataObjects.ManhattenFileType.dbRow saveRow { get; set; }

            private wbm_common.DataObjects.ManhattenFileType.dbRow existRow { get; set; }
            ...
            public ManhattenFileTypeSave(wbm_common.DataObjects.ManhattenFileType saveObject, int UserID, WBMDatabase wBMDB)
            {
            ...
            public Boolean CheckExist()
            {
                ...
                if (saveRow.ID > 0)
                {
                    Result<wbm_common.DataObjects.ManhattenFileType.dbRow> rc = _WBMDB.ManhattenFileType_ReadByID(saveRow.ID).GetAwaiter().GetResult();
                ...
                {
                    Result<wbm_common.DataObjects.ManhattenFileType.dbRow> rcexist = _WBMDB.ManhattenFileType_SearchByID(saveRow.ID).GetAwaiter().GetResult();
                    ...
                    else
                    {
                        ...
                        logger.Error("ManhattenFileTypeSave ID " + saveRow.ID.ToString() + " already exists");
                        ReturnState = 400;
                        SavedSuccessfully = false;
                        return false;
                    }
            ...
            public Boolean CheckForChanges()
            {
                ...
                return ct.CheckForChanges(existRow, saveRow, wbm_common.DataObjects.ManhattenFileType.ListChangeTrackerData);
            }
            ...
            public void SetChanged()
            {
                ...
                else
                {
                    ...
                    changeHeaderRow = WBM_API.Helpers.ChangeTracking.Create_ChangeHeader_dbRow((int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.ManhattenFileType,
            ...
            {
                Result<int> sr = _WBMDB.ManhattenFileType_Insert(saveRow).GetAwaiter().GetResult();
            ...
            public Boolean Save_Update_or_Delete()
            {
                ...
                Result<int> cuResult = _WBMDB.ManhattenFileType_Update(saveRow).GetAwaiter().GetResult();
                ...
                if (cuResult.IsSuccess)
                {
                    ...
                    if (chResult.IsSuccess)
                    {
                        ...
                        int ChangeHeaderTableID = (int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.ManhattenFileType;
