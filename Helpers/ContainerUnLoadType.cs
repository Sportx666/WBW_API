...
{
    public class ContainerUnLoadTypeHelper
    {
        private static Logger logger = LogManager.Setup().LoadConfigurationFromAppSettings().GetCurrentClassLogger();
        public static async Task<Result<List<wbm_common.DataObjects.ContainerUnLoadType.dbRow>>> ExecuteSearch(wbm_common.DataObjects.ContainerUnLoadType.Search _ContainerUnLoadTypeSearchData, WBMDatabase _WBMDB, IHubCallerClients Clients = null, Func<IHubCallerClients, WBMDatabase, string, int, int, wbm_common.DataObjects.LongRunProcessHeader.dbRow, Task<string>> inCallBackStatusFunction = null, wbm_common.DataObjects.LongRunProcessHeader.dbRow longRunProcessHeader = null)
        {
            Result<List<wbm_common.DataObjects.ContainerUnLoadType.dbRow>> rc;
            List<wbm_common.DataObjects.ContainerUnLoadType.dbRow> ContainerUnLoadTypes;
            switch (_ContainerUnLoadTypeSearchData.SearchMode)
            {
                case wbm_common.DataObjects.ContainerUnLoadType.Search.SearchModeType.Undefined:
                    {
                        _ContainerUnLoadTypeSearchData.GenerateSqlQuery();
                        rc = await _WBMDB.ContainerUnLoadType_ListBySql(_ContainerUnLoadTypeSearchData.SqlQuery, _ContainerUnLoadTypeSearchData.ListSqlParameter);
                        break;
                    }
                case wbm_common.DataObjects.ContainerUnLoadType.Search.SearchModeType.SingleID:
                    {
                        rc = await _WBMDB.ContainerUnLoadType_SearchByID(_ContainerUnLoadTypeSearchData.ID);
                        break;
                    }
                case wbm_common.DataObjects.ContainerUnLoadType.Search.SearchModeType.AllRecords:
                    {
                        rc = await _WBMDB.ContainerUnLoadType_List();
                        break;
                    }
                default:
                    {
                        logger.Error("ContainerUnLoadTypeHelper.ExecuteSearch: ContainerUnLoadType Search Mode Type Error");
                        rc = Result.Exception<List<wbm_common.DataObjects.ContainerUnLoadType.dbRow>>("Internal Exception occured");
                        break;
                    }
            }
        ...
        public class ContainerUnLoadTypeSave
        {
            private wbm_common.DataObjects.ContainerUnLoadType saveObject { get; set; }
            private wbm_common.DataObjects.ContainerUnLoadType.dbRow saveRow { get; set; }

            private wbm_common.DataObjects.ContainerUnLoadType.dbRow existRow { get; set; }
            ...
            public ContainerUnLoadTypeSave(wbm_common.DataObjects.ContainerUnLoadType saveObject, int UserID, WBMDatabase wBMDB)
            {
            ...
            public Boolean CheckExist()
            {
                ...
                if (saveRow.ID > 0)
                {
                    Result<wbm_common.DataObjects.ContainerUnLoadType.dbRow> rc = _WBMDB.ContainerUnLoadType_ReadByID(saveRow.ID).GetAwaiter().GetResult();
                ...
                {
                    Result<wbm_common.DataObjects.ContainerUnLoadType.dbRow> rcexist = _WBMDB.ContainerUnLoadType_SearchByPublicID(saveRow.PublicID).GetAwaiter().GetResult();
                    ...
                    else
                    {
                        ...
                        logger.Error("ContainerUnLoadTypeSave ID " + saveRow.ID.ToString() + " PublicID " + saveRow.PublicID + " already exists");
                        ReturnState = 400;
                        SavedSuccessfully = false;
                        return false;
                    }
            ...
            public Boolean CheckForChanges()
            {
                ...
                return ct.CheckForChanges(existRow, saveRow, wbm_common.DataObjects.ContainerUnLoadType.ListChangeTrackerData);
            }
            ...
            public void SetChanged()
            {
                ...
                else
                {
                    ...
                    changeHeaderRow = WBM_API.Helpers.ChangeTracking.Create_ChangeHeader_dbRow((int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.ContainerUnLoadType,
            ...
            {
                Result<int> sr = _WBMDB.ContainerUnLoadType_Insert(saveRow).GetAwaiter().GetResult();
            ...
            public Boolean Save_Update_or_Delete()
            {
                ...
                Result<int> cuResult = _WBMDB.ContainerUnLoadType_Update(saveRow).GetAwaiter().GetResult();
                ...
                if (cuResult.IsSuccess)
                {
                    ...
                    if (chResult.IsSuccess)
                    {
                        ...
                        int ChangeHeaderTableID = (int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.ContainerUnLoadType;
