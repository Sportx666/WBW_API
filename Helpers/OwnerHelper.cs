...
namespace WBM_API.Helpers
{
    public class OwnerHelper
    {
        ...
        public static async Task<Result<List<wbm_common.DataObjects.Owner.dbRow>>> ExecuteSearch(wbm_common.DataObjects.Owner.Search _OwnerSearchData, WBMDatabase _WBMDB, IHubCallerClients Clients = null, Func<IHubCallerClients, WBMDatabase, string, int, int, wbm_common.DataObjects.LongRunProcessHeader.dbRow, Task<string>> inCallBackStatusFunction = null, wbm_common.DataObjects.LongRunProcessHeader.dbRow longRunProcessHeader = null)
        {
            Result<List<wbm_common.DataObjects.Owner.dbRow>> rc;
            List<wbm_common.DataObjects.Owner.dbRow> Owners;
            switch (_OwnerSearchData.SearchMode)
            {
                case wbm_common.DataObjects.Owner.Search.SearchModeType.Undefined:
                    {
                        _OwnerSearchData.GenerateSqlQuery();
                        rc = await _WBMDB.Owner_ListBySql(_OwnerSearchData.SqlQuery, _OwnerSearchData.ListSqlParameter);
                        break;
                    }
                case wbm_common.DataObjects.Owner.Search.SearchModeType.SingleID:
                    {
                        rc = await _WBMDB.OwnerList_SearchByID(_OwnerSearchData.ID);
                        break;
                    }
                case wbm_common.DataObjects.Owner.Search.SearchModeType.AllRecords:
                    {
                        rc = await _WBMDB.Owner_List();
                        break;
                    }
                case wbm_common.DataObjects.Owner.Search.SearchModeType.SingleCompany:
                    {
                        if (_OwnerSearchData.ListCompanyID == null || _OwnerSearchData.ListCompanyID.Count() != 1)
                        {
                            logger.Error("OwnerHelper.ExecuteSearch: Owner SingleCompany doesn't have single company input");
                            rc = Result.Exception<List<wbm_common.DataObjects.Owner.dbRow>>("Internal Exception occured");
                            break;
                        }
                        rc = await _WBMDB.Owner_ListBySingleCompanyID(_OwnerSearchData.ListCompanyID[0]);
                        break;
                    }
                default:
                    {
                        logger.Error("OwnerHelper.ExecuteSearch: Owner Search Mode Type Error");
                        rc = Result.Exception<List<wbm_common.DataObjects.Owner.dbRow>>("Internal Exception occured");
                        break;
                    }
            }
        ...
        public class OwnerSave
        {
            private wbm_common.DataObjects.Owner saveObject { get; set; }
            private wbm_common.DataObjects.Owner.dbRow saveRow { get; set; }

            private wbm_common.DataObjects.Owner.dbRow existRow { get; set; }
            ...
            public OwnerSave(wbm_common.DataObjects.Owner saveObject, int UserID, WBMDatabase wBMDB)
            {
            ...
            public Boolean CheckExist()
            {
                ...
                if (saveRow.ID > 0)
                {
                    Result<wbm_common.DataObjects.Owner.dbRow> rc = _WBMDB.Owner_ReadByID(saveRow.ID).GetAwaiter().GetResult();
                ...
                {
                    Result<wbm_common.DataObjects.Owner.dbRow> rcexist = _WBMDB.Owner_SearchByID(saveRow.ID).GetAwaiter().GetResult();
                    ...
                    else
                    {
                        ...
                        logger.Error("OwnerSave ID " + saveRow.ID.ToString() + " already exists");
                        ReturnState = 400;
                        SavedSuccessfully = false;
                        return false;
                    }
            ...
            public Boolean CheckForChanges()
            {
                ...
                return ct.CheckForChanges(existRow, saveRow, wbm_common.DataObjects.Owner.ListChangeTrackerData);
            }
            ...
            public void SetChanged()
            {
                ...
                else
                {
                    ...
                    changeHeaderRow = WBM_API.Helpers.ChangeTracking.Create_ChangeHeader_dbRow((int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.Owner,
            ...
            {
                Result<int> sr = _WBMDB.Owner_Insert(saveRow).GetAwaiter().GetResult();
            ...
            public Boolean Save_Update_or_Delete()
            {
                ...
                Result<int> cuResult = _WBMDB.Owner_Update(saveRow).GetAwaiter().GetResult();
                ...
                if (cuResult.IsSuccess)
                {
                    ...
                    if (chResult.IsSuccess)
                    {
                        ...
                        int ChangeHeaderTableID = (int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.Owner;
