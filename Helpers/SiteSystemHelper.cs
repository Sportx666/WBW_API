...
{
    public class SiteSystemHelper
    {
        private static Logger logger = LogManager.Setup().LoadConfigurationFromAppSettings().GetCurrentClassLogger();
        public static async Task<Result<List<wbm_common.DataObjects.SiteSystem.dbRow>>> ExecuteSearch(wbm_common.DataObjects.SiteSystem.Search _SiteSystemSearchData, WBMDatabase _WBMDB, IHubCallerClients Clients = null, Func<IHubCallerClients, WBMDatabase, string, int, int, wbm_common.DataObjects.LongRunProcessHeader.dbRow, Task<string>> inCallBackStatusFunction = null, wbm_common.DataObjects.LongRunProcessHeader.dbRow longRunProcessHeader = null)
        {
            Result<List<wbm_common.DataObjects.SiteSystem.dbRow>> rc;
            List<wbm_common.DataObjects.SiteSystem.dbRow> SiteSystems;
            switch (_SiteSystemSearchData.SearchMode)
            {
                case wbm_common.DataObjects.SiteSystem.Search.SearchModeType.Undefined:
                    {
                        _SiteSystemSearchData.GenerateSqlQuery();
                        rc = await _WBMDB.SiteSystem_ListBySql(_SiteSystemSearchData.SqlQuery, _SiteSystemSearchData.ListSqlParameter);
                        break;
                    }
                case wbm_common.DataObjects.SiteSystem.Search.SearchModeType.SingleID:
                    {
                        rc = await _WBMDB.SiteSystemList_SearchByID(_SiteSystemSearchData.ID);
                        break;
                    }
                case wbm_common.DataObjects.SiteSystem.Search.SearchModeType.AllRecords:
                    {
                        rc = await _WBMDB.SiteSystem_List();
                        break;
                    }
                default:
                    {
                        logger.Error("SiteSystemHelper.ExecuteSearch: SiteSystem Search Mode Type Error");
                        rc = Result.Exception<List<wbm_common.DataObjects.SiteSystem.dbRow>>("Internal Exception occured");
                        break;
                    }
            }
        ...
        public class SiteSystemSave
        {
            private wbm_common.DataObjects.SiteSystem saveObject { get; set; }
            private wbm_common.DataObjects.SiteSystem.dbRow saveRow { get; set; }

            private wbm_common.DataObjects.SiteSystem.dbRow existRow { get; set; }
            ...
            public SiteSystemSave(wbm_common.DataObjects.SiteSystem saveObject, int UserID, WBMDatabase wBMDB)
            {
            ...
            public Boolean CheckExist()
            {
                ...
                if (saveRow.ID > 0)
                {
                    Result<wbm_common.DataObjects.SiteSystem.dbRow> rc = _WBMDB.SiteSystem_ReadByID(saveRow.ID).GetAwaiter().GetResult();
                ...
                {
                    Result<wbm_common.DataObjects.SiteSystem.dbRow> rcexist = _WBMDB.SiteSystem_SearchByID(saveRow.ID).GetAwaiter().GetResult();
                    ...
                    else
                    {
                        ...
                        logger.Error("SiteSystemSave ID " + saveRow.ID.ToString() + " already exists");
                        ReturnState = 400;
                        SavedSuccessfully = false;
                        return false;
                    }
            ...
            public Boolean CheckForChanges()
            {
                ...
                return ct.CheckForChanges(existRow, saveRow, wbm_common.DataObjects.SiteSystem.ListChangeTrackerData);
            }
            ...
            public void SetChanged()
            {
                ...
                else
                {
                    ...
                    changeHeaderRow = WBM_API.Helpers.ChangeTracking.Create_ChangeHeader_dbRow((int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.SiteSystem,
            ...
            {
                Result<int> sr = _WBMDB.SiteSystem_Insert(saveRow).GetAwaiter().GetResult();
            ...
            public Boolean Save_Update_or_Delete()
            {
                ...
                Result<int> cuResult = _WBMDB.SiteSystem_Update(saveRow).GetAwaiter().GetResult();
                ...
                if (cuResult.IsSuccess)
                {
                    ...
                    if (chResult.IsSuccess)
                    {
                        ...
                        int ChangeHeaderTableID = (int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.SiteSystem;
