...
{
    public class ContainerSizeHelper
    {
        private static Logger logger = LogManager.Setup().LoadConfigurationFromAppSettings().GetCurrentClassLogger();
        public static async Task<Result<List<wbm_common.DataObjects.ContainerSize.dbRow>>> ExecuteSearch(wbm_common.DataObjects.ContainerSize.Search _ContainerSizeSearchData, WBMDatabase _WBMDB, IHubCallerClients Clients = null, Func<IHubCallerClients, WBMDatabase, string, int, int, wbm_common.DataObjects.LongRunProcessHeader.dbRow, Task<string>> inCallBackStatusFunction = null, wbm_common.DataObjects.LongRunProcessHeader.dbRow longRunProcessHeader = null)
        {
            Result<List<wbm_common.DataObjects.ContainerSize.dbRow>> rc;
            List<wbm_common.DataObjects.ContainerSize.dbRow> ContainerSizes;
            switch (_ContainerSizeSearchData.SearchMode)
            {
                case wbm_common.DataObjects.ContainerSize.Search.SearchModeType.Undefined:
                    {
                        _ContainerSizeSearchData.GenerateSqlQuery();
                        rc = await _WBMDB.ContainerSize_ListBySql(_ContainerSizeSearchData.SqlQuery, _ContainerSizeSearchData.ListSqlParameter);
                        break;
                    }
                case wbm_common.DataObjects.ContainerSize.Search.SearchModeType.SingleID:
                    {
                        rc = await _WBMDB.ContainerSize_SearchByID(_ContainerSizeSearchData.ID);
                        break;
                    }
                case wbm_common.DataObjects.ContainerSize.Search.SearchModeType.AllRecords:
                    {
                        rc = await _WBMDB.ContainerSize_List();
                        break;
                    }
                default:
                    {
                        logger.Error("ContainerSizeHelper.ExecuteSearch: ContainerSize Search Mode Type Error");
                        rc = Result.Exception<List<wbm_common.DataObjects.ContainerSize.dbRow>>("Internal Exception occured");
                        break;
                    }
            }
        ...
        public class ContainerSizeSave
        {
            private wbm_common.DataObjects.ContainerSize saveObject { get; set; }
            private wbm_common.DataObjects.ContainerSize.dbRow saveRow { get; set; }

            private wbm_common.DataObjects.ContainerSize.dbRow existRow { get; set; }
            ...
            public ContainerSizeSave(wbm_common.DataObjects.ContainerSize saveObject, int UserID, WBMDatabase wBMDB)
            {
            ...
            public Boolean CheckExist()
            {
                ...
                if (saveRow.ID > 0)
                {
                    Result<wbm_common.DataObjects.ContainerSize.dbRow> rc = _WBMDB.ContainerSize_ReadByID(saveRow.ID).GetAwaiter().GetResult();
                ...
                {
                    Result<wbm_common.DataObjects.ContainerSize.dbRow> rcexist = _WBMDB.ContainerSize_SearchByPublicID(saveRow.PublicID).GetAwaiter().GetResult();
                    ...
                    else
                    {
                        ...
                        logger.Error("ContainerSizeSave ID " + saveRow.ID.ToString() + " PublicID " + saveRow.PublicID + " already exists");
                        ReturnState = 400;
                        SavedSuccessfully = false;
                        return false;
                    }
            ...
            public Boolean CheckForChanges()
            {
                ...
                return ct.CheckForChanges(existRow, saveRow, wbm_common.DataObjects.ContainerSize.ListChangeTrackerData);
            }
            ...
            public void SetChanged()
            {
                ...
                else
                {
                    ...
                    changeHeaderRow = WBM_API.Helpers.ChangeTracking.Create_ChangeHeader_dbRow((int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.ContainerSize,
            ...
            {
                Result<int> sr = _WBMDB.ContainerSize_Insert(saveRow).GetAwaiter().GetResult();
            ...
            public Boolean Save_Update_or_Delete()
            {
                ...
                Result<int> cuResult = _WBMDB.ContainerSize_Update(saveRow).GetAwaiter().GetResult();
                ...
                if (cuResult.IsSuccess)
                {
                    ...
                    if (chResult.IsSuccess)
                    {
                        ...
                        int ChangeHeaderTableID = (int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.ContainerSize;
