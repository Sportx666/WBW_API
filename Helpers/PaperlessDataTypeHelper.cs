...
{
    public class PaperlessDataTypeHelper
    {
        private static Logger logger = LogManager.Setup().LoadConfigurationFromAppSettings().GetCurrentClassLogger();
        public static async Task<Result<List<wbm_common.DataObjects.PaperlessDataType.dbRow>>> ExecuteSearch(wbm_common.DataObjects.PaperlessDataType.Search _PaperlessDataTypeSearchData, WBMDatabase _WBMDB, IHubCallerClients Clients = null, Func<IHubCallerClients, WBMDatabase, string, int, int, wbm_common.DataObjects.LongRunProcessHeader.dbRow, Task<string>> inCallBackStatusFunction = null, wbm_common.DataObjects.LongRunProcessHeader.dbRow longRunProcessHeader = null)
        {
            Result<List<wbm_common.DataObjects.PaperlessDataType.dbRow>> rc;
            List<wbm_common.DataObjects.PaperlessDataType.dbRow> PaperlessDataTypes;
            switch (_PaperlessDataTypeSearchData.SearchMode)
            {
                case wbm_common.DataObjects.PaperlessDataType.Search.SearchModeType.Undefined:
                    {
                        _PaperlessDataTypeSearchData.GenerateSqlQuery();
                        rc = await _WBMDB.PaperlessDataType_ListBySql(_PaperlessDataTypeSearchData.SqlQuery, _PaperlessDataTypeSearchData.ListSqlParameter);
                        break;
                    }
                case wbm_common.DataObjects.PaperlessDataType.Search.SearchModeType.SingleID:
                    {
                        rc = await _WBMDB.PaperlessDataTypeList_SearchByID(_PaperlessDataTypeSearchData.ID);
                        break;
                    }
                case wbm_common.DataObjects.PaperlessDataType.Search.SearchModeType.AllRecords:
                    {
                        rc = await _WBMDB.PaperlessDataType_List();
                        break;
                    }
                default:
                    {
                        logger.Error("PaperlessDataTypeHelper.ExecuteSearch: PaperlessDataType Search Mode Type Error");
                        rc = Result.Exception<List<wbm_common.DataObjects.PaperlessDataType.dbRow>>("Internal Exception occured");
                        break;
                    }
            }
        ...
        public class PaperlessDataTypeSave
        {
            private wbm_common.DataObjects.PaperlessDataType saveObject { get; set; }
            private wbm_common.DataObjects.PaperlessDataType.dbRow saveRow { get; set; }

            private wbm_common.DataObjects.PaperlessDataType.dbRow existRow { get; set; }
            ...
            public PaperlessDataTypeSave(wbm_common.DataObjects.PaperlessDataType saveObject, int UserID, WBMDatabase wBMDB)
            {
            ...
            public Boolean CheckExist()
            {
                ...
                if (saveRow.ID > 0)
                {
                    Result<wbm_common.DataObjects.PaperlessDataType.dbRow> rc = _WBMDB.PaperlessDataType_ReadByID(saveRow.ID).GetAwaiter().GetResult();
                ...
                {
                    Result<wbm_common.DataObjects.PaperlessDataType.dbRow> rcexist = _WBMDB.PaperlessDataType_SearchByID(saveRow.ID).GetAwaiter().GetResult();
                    ...
                    else
                    {
                        ...
                        logger.Error("PaperlessDataTypeSave ID " + saveRow.ID.ToString() + " already exists");
                        ReturnState = 400;
                        SavedSuccessfully = false;
                        return false;
                    }
            ...
            public Boolean CheckForChanges()
            {
                ...
                return ct.CheckForChanges(existRow, saveRow, wbm_common.DataObjects.PaperlessDataType.ListChangeTrackerData);
            }
            ...
            public void SetChanged()
            {
                ...
                else
                {
                    ...
                    changeHeaderRow = WBM_API.Helpers.ChangeTracking.Create_ChangeHeader_dbRow((int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.PaperlessDataType,
            ...
            {
                Result<int> sr = _WBMDB.PaperlessDataType_Insert(saveRow).GetAwaiter().GetResult();
            ...
            public Boolean Save_Update_or_Delete()
            {
                ...
                Result<int> cuResult = _WBMDB.PaperlessDataType_Update(saveRow).GetAwaiter().GetResult();
                ...
                if (cuResult.IsSuccess)
                {
                    ...
                    if (chResult.IsSuccess)
                    {
                        ...
                        int ChangeHeaderTableID = (int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.PaperlessDataType;
