...
namespace WBM_API.Helpers
{
    public class NoteTypeHelper
    {
        ...
        public static async Task<Result<List<wbm_common.DataObjects.NoteType.dbRow>>> ExecuteSearch(wbm_common.DataObjects.NoteType.Search _NoteTypeSearchData, WBMDatabase _WBMDB, IHubCallerClients Clients = null, Func<IHubCallerClients, WBMDatabase, string, int, int, wbm_common.DataObjects.LongRunProcessHeader.dbRow, Task<string>> inCallBackStatusFunction = null, wbm_common.DataObjects.LongRunProcessHeader.dbRow longRunProcessHeader = null)
        {
            Result<List<wbm_common.DataObjects.NoteType.dbRow>> rc;
            List<wbm_common.DataObjects.NoteType.dbRow> NoteTypes;
            switch (_NoteTypeSearchData.SearchMode)
            {
                case wbm_common.DataObjects.NoteType.Search.SearchModeType.Undefined:
                    {
                        _NoteTypeSearchData.GenerateSqlQuery();
                        rc = await _WBMDB.NoteType_ListBySql(_NoteTypeSearchData.SqlQuery, _NoteTypeSearchData.ListSqlParameter);
                        break;
                    }
                case wbm_common.DataObjects.NoteType.Search.SearchModeType.SingleID:
                    {
                        rc = await _WBMDB.NoteTypeList_SearchByID(_NoteTypeSearchData.ID);
                        break;
                    }
                case wbm_common.DataObjects.NoteType.Search.SearchModeType.AllRecords:
                    {
                        rc = await _WBMDB.NoteType_List();
                        break;
                    }
                default:
                    {
                        logger.Error("NoteTypeHelper.ExecuteSearch: NoteType Search Mode Type Error");
                        rc = Result.Exception<List<wbm_common.DataObjects.NoteType.dbRow>>("Internal Exception occured");
                        break;
                    }
            }
        ...
        public class NoteTypeSave
        {
            private wbm_common.DataObjects.NoteType saveObject { get; set; }
            private wbm_common.DataObjects.NoteType.dbRow saveRow { get; set; }

            private wbm_common.DataObjects.NoteType.dbRow existRow { get; set; }
            ...
            public NoteTypeSave(wbm_common.DataObjects.NoteType saveObject, int UserID, WBMDatabase wBMDB)
            {
            ...
            public Boolean CheckExist()
            {
                ...
                if (saveRow.ID > 0)
                {
                    Result<wbm_common.DataObjects.NoteType.dbRow> rc = _WBMDB.NoteType_ReadByID(saveRow.ID).GetAwaiter().GetResult();
                ...
                {
                    Result<wbm_common.DataObjects.NoteType.dbRow> rcexist = _WBMDB.NoteType_SearchByID(saveRow.ID).GetAwaiter().GetResult();
                    ...
                    else
                    {
                        ...
                        logger.Error("NoteTypeSave ID " + saveRow.ID.ToString() + " already exists");
                        ReturnState = 400;
                        SavedSuccessfully = false;
                        return false;
                    }
            ...
            public Boolean CheckForChanges()
            {
                ...
                return ct.CheckForChanges(existRow, saveRow, wbm_common.DataObjects.NoteType.ListChangeTrackerData);
            }
            ...
            public void SetChanged()
            {
                ...
                else
                {
                    ...
                    changeHeaderRow = WBM_API.Helpers.ChangeTracking.Create_ChangeHeader_dbRow((int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.NoteType,
            ...
            {
                Result<int> sr = _WBMDB.NoteType_Insert(saveRow).GetAwaiter().GetResult();
            ...
            public Boolean Save_Update_or_Delete()
            {
                ...
                Result<int> cuResult = _WBMDB.NoteType_Update(saveRow).GetAwaiter().GetResult();
                ...
                if (cuResult.IsSuccess)
                {
                    ...
                    if (chResult.IsSuccess)
                    {
                        ...
                        int ChangeHeaderTableID = (int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.NoteType;
