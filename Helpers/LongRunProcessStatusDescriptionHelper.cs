...
{
    public class LongRunProcessStatusDescriptionHelper
    {
        private static Logger logger = LogManager.Setup().LoadConfigurationFromAppSettings().GetCurrentClassLogger();
        public static async Task<Result<List<wbm_common.DataObjects.LongRunProcessStatusDescription.dbRow>>> ExecuteSearch(wbm_common.DataObjects.LongRunProcessStatusDescription.Search _LongRunProcessStatusDescriptionSearchData, WBMDatabase _WBMDB, IHubCallerClients Clients = null, Func<IHubCallerClients, WBMDatabase, string, int, int, wbm_common.DataObjects.LongRunProcessHeader.dbRow, Task<string>> inCallBackStatusFunction = null, wbm_common.DataObjects.LongRunProcessHeader.dbRow longRunProcessHeader = null)
        {
            Result<List<wbm_common.DataObjects.LongRunProcessStatusDescription.dbRow>> rc;
            List<wbm_common.DataObjects.LongRunProcessStatusDescription.dbRow> LongRunProcessStatusDescriptions;
            switch (_LongRunProcessStatusDescriptionSearchData.SearchMode)
            {
                case wbm_common.DataObjects.LongRunProcessStatusDescription.Search.SearchModeType.Undefined:
                    {
                        _LongRunProcessStatusDescriptionSearchData.GenerateSqlQuery();
                        rc = await _WBMDB.LongRunProcessStatusDescription_ListBySql(_LongRunProcessStatusDescriptionSearchData.SqlQuery, _LongRunProcessStatusDescriptionSearchData.ListSqlParameter);
                        break;
                    }
                case wbm_common.DataObjects.LongRunProcessStatusDescription.Search.SearchModeType.SingleID:
                    {
                        rc = await _WBMDB.LongRunProcessStatusDescriptionList_SearchByID(_LongRunProcessStatusDescriptionSearchData.ID);
                        break;
                    }
                case wbm_common.DataObjects.LongRunProcessStatusDescription.Search.SearchModeType.AllRecords:
                    {
                        rc = await _WBMDB.LongRunProcessStatusDescription_List();
                        break;
                    }
                default:
                    {
                        logger.Error("LongRunProcessStatusDescriptionHelper.ExecuteSearch: LongRunProcessStatusDescription Search Mode Type Error");
                        rc = Result.Exception<List<wbm_common.DataObjects.LongRunProcessStatusDescription.dbRow>>("Internal Exception occured");
                        break;
                    }
            }
        ...
        public class LongRunProcessStatusDescriptionSave
        {
            private wbm_common.DataObjects.LongRunProcessStatusDescription saveObject { get; set; }
            private wbm_common.DataObjects.LongRunProcessStatusDescription.dbRow saveRow { get; set; }

            private wbm_common.DataObjects.LongRunProcessStatusDescription.dbRow existRow { get; set; }
            ...
            public LongRunProcessStatusDescriptionSave(wbm_common.DataObjects.LongRunProcessStatusDescription saveObject, int UserID, WBMDatabase wBMDB)
            {
            ...
            public Boolean CheckExist()
            {
                ...
                if (saveRow.ID > 0)
                {
                    Result<wbm_common.DataObjects.LongRunProcessStatusDescription.dbRow> rc = _WBMDB.LongRunProcessStatusDescription_ReadByID(saveRow.ID).GetAwaiter().GetResult();
                ...
                {
                    Result<wbm_common.DataObjects.LongRunProcessStatusDescription.dbRow> rcexist = _WBMDB.LongRunProcessStatusDescription_SearchByID(saveRow.ID).GetAwaiter().GetResult();
                    ...
                    else
                    {
                        ...
                        logger.Error("LongRunProcessStatusDescriptionSave ID " + saveRow.ID.ToString() + " already exists");
                        ReturnState = 400;
                        SavedSuccessfully = false;
                        return false;
                    }
            ...
            public Boolean CheckForChanges()
            {
                ...
                return ct.CheckForChanges(existRow, saveRow, wbm_common.DataObjects.LongRunProcessStatusDescription.ListChangeTrackerData);
            }
            ...
            public void SetChanged()
            {
                ...
                else
                {
                    ...
                    changeHeaderRow = WBM_API.Helpers.ChangeTracking.Create_ChangeHeader_dbRow((int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.LongRunProcessStatusDescription,
            ...
            {
                Result<int> sr = _WBMDB.LongRunProcessStatusDescription_Insert(saveRow).GetAwaiter().GetResult();
            ...
            public Boolean Save_Update_or_Delete()
            {
                ...
                Result<int> cuResult = _WBMDB.LongRunProcessStatusDescription_Update(saveRow).GetAwaiter().GetResult();
                ...
                if (cuResult.IsSuccess)
                {
                    ...
                    if (chResult.IsSuccess)
                    {
                        ...
                        int ChangeHeaderTableID = (int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.LongRunProcessStatusDescription;
