...
{
    public class InvoiceCycleHelper
    {
        private static Logger logger = LogManager.Setup().LoadConfigurationFromAppSettings().GetCurrentClassLogger();
        public static async Task<Result<List<wbm_common.DataObjects.InvoiceCycle.dbRow>>> ExecuteSearch(wbm_common.DataObjects.InvoiceCycle.Search _InvoiceCycleSearchData, WBMDatabase _WBMDB, IHubCallerClients Clients = null, Func<IHubCallerClients, WBMDatabase, string, int, int, wbm_common.DataObjects.LongRunProcessHeader.dbRow, Task<string>> inCallBackStatusFunction = null, wbm_common.DataObjects.LongRunProcessHeader.dbRow longRunProcessHeader = null)
        {
            Result<List<wbm_common.DataObjects.InvoiceCycle.dbRow>> rc;
            List<wbm_common.DataObjects.InvoiceCycle.dbRow> InvoiceCycles;
            switch (_InvoiceCycleSearchData.SearchMode)
            {
                case wbm_common.DataObjects.InvoiceCycle.Search.SearchModeType.Undefined:
                    {
                        _InvoiceCycleSearchData.GenerateSqlQuery();
                        rc = await _WBMDB.InvoiceCycle_ListBySql(_InvoiceCycleSearchData.SqlQuery, _InvoiceCycleSearchData.ListSqlParameter);
                        break;
                    }
                case wbm_common.DataObjects.InvoiceCycle.Search.SearchModeType.SingleID:
                    {
                        rc = await _WBMDB.InvoiceCycleList_SearchByID(_InvoiceCycleSearchData.ID);
                        break;
                    }
                case wbm_common.DataObjects.InvoiceCycle.Search.SearchModeType.AllRecords:
                    {
                        rc = await _WBMDB.InvoiceCycle_List();
                        break;
                    }
                default:
                    {
                        logger.Error("InvoiceCycleHelper.ExecuteSearch: InvoiceCycle Search Mode Type Error");
                        rc = Result.Exception<List<wbm_common.DataObjects.InvoiceCycle.dbRow>>("Internal Exception occured");
                        break;
                    }
            }
        ...
        public class InvoiceCycleSave
        {
            private wbm_common.DataObjects.InvoiceCycle saveObject { get; set; }
            private wbm_common.DataObjects.InvoiceCycle.dbRow saveRow { get; set; }

            private wbm_common.DataObjects.InvoiceCycle.dbRow existRow { get; set; }
            ...
            public InvoiceCycleSave(wbm_common.DataObjects.InvoiceCycle saveObject, int UserID, WBMDatabase wBMDB)
            {
            ...
            public Boolean CheckExist()
            {
                ...
                if (saveRow.ID > 0)
                {
                    Result<wbm_common.DataObjects.InvoiceCycle.dbRow> rc = _WBMDB.InvoiceCycle_ReadByID(saveRow.ID).GetAwaiter().GetResult();
                ...
                {
                    Result<wbm_common.DataObjects.InvoiceCycle.dbRow> rcexist = _WBMDB.InvoiceCycle_SearchByID(saveRow.ID).GetAwaiter().GetResult();
                    ...
                    else
                    {
                        ...
                        logger.Error("InvoiceCycleSave ID " + saveRow.ID.ToString() + " already exists");
                        ReturnState = 400;
                        SavedSuccessfully = false;
                        return false;
                    }
            ...
            public Boolean CheckForChanges()
            {
                ...
                return ct.CheckForChanges(existRow, saveRow, wbm_common.DataObjects.InvoiceCycle.ListChangeTrackerData);
            }
            ...
            public void SetChanged()
            {
                ...
                else
                {
                    ...
                    changeHeaderRow = WBM_API.Helpers.ChangeTracking.Create_ChangeHeader_dbRow((int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.InvoiceCycle,
            ...
            {
                Result<int> sr = _WBMDB.InvoiceCycle_Insert(saveRow).GetAwaiter().GetResult();
            ...
            public Boolean Save_Update_or_Delete()
            {
                ...
                Result<int> cuResult = _WBMDB.InvoiceCycle_Update(saveRow).GetAwaiter().GetResult();
                ...
                if (cuResult.IsSuccess)
                {
                    ...
                    if (chResult.IsSuccess)
                    {
                        ...
                        int ChangeHeaderTableID = (int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.InvoiceCycle;
