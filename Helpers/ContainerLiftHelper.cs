...
{
    public class ContainerLiftHelper
    {
        private static Logger logger = LogManager.Setup().LoadConfigurationFromAppSettings().GetCurrentClassLogger();
        public static async Task<Result<List<wbm_common.DataObjects.ContainerLift.dbRow>>> ExecuteSearch(wbm_common.DataObjects.ContainerLift.Search _ContainerLiftSearchData, WBMDatabase _WBMDB, IHubCallerClients Clients = null, Func<IHubCallerClients, WBMDatabase, string, int, int, wbm_common.DataObjects.LongRunProcessHeader.dbRow, Task<string>> inCallBackStatusFunction = null, wbm_common.DataObjects.LongRunProcessHeader.dbRow longRunProcessHeader = null)
        {
            Result<List<wbm_common.DataObjects.ContainerLift.dbRow>> rc;
            List<wbm_common.DataObjects.ContainerLift.dbRow> ContainerLifts;
            switch (_ContainerLiftSearchData.SearchMode)
            {
                case wbm_common.DataObjects.ContainerLift.Search.SearchModeType.Undefined:
                    {
                        _ContainerLiftSearchData.GenerateSqlQuery();
                        rc = await _WBMDB.ContainerLift_ListBySql(_ContainerLiftSearchData.SqlQuery, _ContainerLiftSearchData.ListSqlParameter);
                        break;
                    }
                case wbm_common.DataObjects.ContainerLift.Search.SearchModeType.SingleID:
                    {
                        rc = await _WBMDB.ContainerLiftList_SearchByID(_ContainerLiftSearchData.ID);
                        break;
                    }
                case wbm_common.DataObjects.ContainerLift.Search.SearchModeType.AllRecords:
                    {
                        rc = await _WBMDB.ContainerLift_List();
                        break;
                    }
                case wbm_common.DataObjects.ContainerLift.Search.SearchModeType.SingleOwnerID:
                    {
                        if (_ContainerLiftSearchData.ListOwnerID == null || _ContainerLiftSearchData.ListOwnerID.Count() != 1)
                        {
                            logger.Error("ContainerLiftHelper.ExecuteSearch: ActionRateCode SingleOwner doesn't have single owner input");
                            rc = Result.Exception<List<wbm_common.DataObjects.ContainerLift.dbRow>>("Internal Exception occured");
                            break;
                        }
                        rc = await _WBMDB.ContainerLift_ListByOwnerID(_ContainerLiftSearchData.ListOwnerID[0]);
                        break;
                    }
                default:
                    {
                        logger.Error("ContainerLiftHelper.ExecuteSearch: ContainerLift Search Mode Type Error");
                        rc = Result.Exception<List<wbm_common.DataObjects.ContainerLift.dbRow>>("Internal Exception occured");
                        break;
                    }
            }
        ...
        public class ContainerLiftSave
        {
            private wbm_common.DataObjects.ContainerLift saveObject { get; set; }
            private wbm_common.DataObjects.ContainerLift.dbRow saveRow { get; set; }

            private wbm_common.DataObjects.ContainerLift.dbRow existRow { get; set; }
            ...
            public ContainerLiftSave(wbm_common.DataObjects.ContainerLift saveObject, int UserID, WBMDatabase wBMDB)
            {
            ...
            public Boolean CheckExist()
            {
                ...
                if (saveRow.ID > 0)
                {
                    Result<wbm_common.DataObjects.ContainerLift.dbRow> rc = _WBMDB.ContainerLift_ReadByID(saveRow.ID).GetAwaiter().GetResult();
                ...
                {
                    Result<wbm_common.DataObjects.ContainerLift.dbRow> rcexist = _WBMDB.ContainerLift_SearchByID(saveRow.ID).GetAwaiter().GetResult();
                    ...
                    else
                    {
                        ...
                        logger.Error("ContainerLiftSave ID " + saveRow.ID.ToString() + " already exists");
                        ReturnState = 400;
                        SavedSuccessfully = false;
                        return false;
                    }
            ...
            public Boolean CheckForChanges()
            {
                ...
                return ct.CheckForChanges(existRow, saveRow, wbm_common.DataObjects.ContainerLift.ListChangeTrackerData);
            }
            ...
            public void SetChanged()
            {
                ...
                else
                {
                    ...
                    changeHeaderRow = WBM_API.Helpers.ChangeTracking.Create_ChangeHeader_dbRow((int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.ContainerLift,
            ...
            {
                Result<int> sr = _WBMDB.ContainerLift_Insert(saveRow).GetAwaiter().GetResult();
            ...
            public Boolean Save_Update_or_Delete()
            {
                ...
                Result<int> cuResult = _WBMDB.ContainerLift_Update(saveRow).GetAwaiter().GetResult();
                ...
                if (cuResult.IsSuccess)
                {
                    ...
                    if (chResult.IsSuccess)
                    {
                        ...
                        int ChangeHeaderTableID = (int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.ContainerLift;
