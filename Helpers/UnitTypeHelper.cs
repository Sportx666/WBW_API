...
namespace WBM_API.Helpers
{
    public class UnitTypeHelper
    {
        ...
        public static async Task<Result<List<wbm_common.DataObjects.UnitType.dbRow>>> ExecuteSearch(wbm_common.DataObjects.UnitType.Search _UnitTypeSearchData, WBMDatabase _WBMDB, IHubCallerClients Clients = null, Func<IHubCallerClients, WBMDatabase, string, int, int, wbm_common.DataObjects.LongRunProcessHeader.dbRow, Task<string>> inCallBackStatusFunction = null, wbm_common.DataObjects.LongRunProcessHeader.dbRow longRunProcessHeader = null)
        {
            Result<List<wbm_common.DataObjects.UnitType.dbRow>> rc;
            List<wbm_common.DataObjects.UnitType.dbRow> UnitTypes;
            switch (_UnitTypeSearchData.SearchMode)
            {
                case wbm_common.DataObjects.UnitType.Search.SearchModeType.Undefined:
                    {
                        _UnitTypeSearchData.GenerateSqlQuery();
                        rc = await _WBMDB.UnitType_ListBySql(_UnitTypeSearchData.SqlQuery, _UnitTypeSearchData.ListSqlParameter);
                        break;
                    }
                case wbm_common.DataObjects.UnitType.Search.SearchModeType.SingleID:
                    {
                        rc = await _WBMDB.UnitTypeList_SearchByID(_UnitTypeSearchData.ID);
                        break;
                    }
                case wbm_common.DataObjects.UnitType.Search.SearchModeType.AllRecords:
                    {
                        rc = await _WBMDB.UnitType_List();
                        break;
                    }
                default:
                    {
                        logger.Error("UnitTypeHelper.ExecuteSearch: UnitType Search Mode Type Error");
                        rc = Result.Exception<List<wbm_common.DataObjects.UnitType.dbRow>>("Internal Exception occured");
                        break;
                    }
            }
        ...
        public class UnitTypeSave
        {
            private wbm_common.DataObjects.UnitType saveObject { get; set; }
            private wbm_common.DataObjects.UnitType.dbRow saveRow { get; set; }

            private wbm_common.DataObjects.UnitType.dbRow existRow { get; set; }
            ...
            public UnitTypeSave(wbm_common.DataObjects.UnitType saveObject, int UserID, WBMDatabase wBMDB)
            {
            ...
            public Boolean CheckExist()
            {
                ...
                if (saveRow.ID > 0)
                {
                    Result<wbm_common.DataObjects.UnitType.dbRow> rc = _WBMDB.UnitType_ReadByID(saveRow.ID).GetAwaiter().GetResult();
                ...
                {
                    Result<wbm_common.DataObjects.UnitType.dbRow> rcexist = _WBMDB.UnitType_SearchByID(saveRow.ID).GetAwaiter().GetResult();
                    ...
                    else
                    {
                        ...
                        logger.Error("UnitTypeSave ID " + saveRow.ID.ToString() + " already exists");
                        ReturnState = 400;
                        SavedSuccessfully = false;
                        return false;
                    }
            ...
            public Boolean CheckForChanges()
            {
                ...
                return ct.CheckForChanges(existRow, saveRow, wbm_common.DataObjects.UnitType.ListChangeTrackerData);
            }
            ...
            public void SetChanged()
            {
                ...
                else
                {
                    ...
                    changeHeaderRow = WBM_API.Helpers.ChangeTracking.Create_ChangeHeader_dbRow((int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.UnitType,
            ...
            {
                Result<int> sr = _WBMDB.UnitType_Insert(saveRow).GetAwaiter().GetResult();
            ...
            public Boolean Save_Update_or_Delete()
            {
                ...
                Result<int> cuResult = _WBMDB.UnitType_Update(saveRow).GetAwaiter().GetResult();
                ...
                if (cuResult.IsSuccess)
                {
                    ...
                    if (chResult.IsSuccess)
                    {
                        ...
                        int ChangeHeaderTableID = (int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.UnitType;
