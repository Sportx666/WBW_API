...
{
    public class ProcessingRuleHelper
    {
        private static Logger logger = LogManager.Setup().LoadConfigurationFromAppSettings().GetCurrentClassLogger();
        public static async Task<Result<List<wbm_common.DataObjects.ProcessingRule.dbRow>>> ExecuteSearch(wbm_common.DataObjects.ProcessingRule.Search _ProcessingRuleSearchData, WBMDatabase _WBMDB, IHubCallerClients Clients = null, Func<IHubCallerClients, WBMDatabase, string, int, int, wbm_common.DataObjects.LongRunProcessHeader.dbRow, Task<string>> inCallBackStatusFunction = null, wbm_common.DataObjects.LongRunProcessHeader.dbRow longRunProcessHeader = null)
        {
            Result<List<wbm_common.DataObjects.ProcessingRule.dbRow>> rc;
            List<wbm_common.DataObjects.ProcessingRule.dbRow> ProcessingRules;
            switch (_ProcessingRuleSearchData.SearchMode)
            {
                case wbm_common.DataObjects.ProcessingRule.Search.SearchModeType.Undefined:
                    {
                        _ProcessingRuleSearchData.GenerateSqlQuery();
                        rc = await _WBMDB.ProcessingRule_ListBySql(_ProcessingRuleSearchData.SqlQuery, _ProcessingRuleSearchData.ListSqlParameter);
                        break;
                    }
                case wbm_common.DataObjects.ProcessingRule.Search.SearchModeType.SingleID:
                    {
                        rc = await _WBMDB.ProcessingRuleList_SearchByID(_ProcessingRuleSearchData.ID);
                        break;
                    }
                case wbm_common.DataObjects.ProcessingRule.Search.SearchModeType.AllRecords:
                    {
                        rc = await _WBMDB.ProcessingRule_List();
                        break;
                    }
                default:
                    {
                        logger.Error("ProcessingRuleHelper.ExecuteSearch: ProcessingRule Search Mode Type Error");
                        rc = Result.Exception<List<wbm_common.DataObjects.ProcessingRule.dbRow>>("Internal Exception occured");
                        break;
                    }
            }
        ...
        public class ProcessingRuleSave
        {
            private wbm_common.DataObjects.ProcessingRule saveObject { get; set; }
            private wbm_common.DataObjects.ProcessingRule.dbRow saveRow { get; set; }

            private wbm_common.DataObjects.ProcessingRule.dbRow existRow { get; set; }
            ...
            public ProcessingRuleSave(wbm_common.DataObjects.ProcessingRule saveObject, int UserID, WBMDatabase wBMDB)
            {
            ...
            public Boolean CheckExist()
            {
                ...
                if (saveRow.ID > 0)
                {
                    Result<wbm_common.DataObjects.ProcessingRule.dbRow> rc = _WBMDB.ProcessingRule_ReadByID(saveRow.ID).GetAwaiter().GetResult();
                ...
                {
                    Result<wbm_common.DataObjects.ProcessingRule.dbRow> rcexist = _WBMDB.ProcessingRule_SearchByID(saveRow.ID).GetAwaiter().GetResult();
                    ...
                    else
                    {
                        ...
                        logger.Error("ProcessingRuleSave ID " + saveRow.ID + " already exists");
                        ReturnState = 400;
                        SavedSuccessfully = false;
                        return false;
                    }
            ...
            public Boolean CheckForChanges()
            {
                ...
                return ct.CheckForChanges(existRow, saveRow, wbm_common.DataObjects.ProcessingRule.ListChangeTrackerData);
            }
            ...
            public void SetChanged()
            {
                ...
                else
                {
                    ...
                    changeHeaderRow = WBM_API.Helpers.ChangeTracking.Create_ChangeHeader_dbRow((int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.ProcessingRule,
            ...
            {
                Result<int> sr = _WBMDB.ProcessingRule_Insert(saveRow).GetAwaiter().GetResult();
            ...
            public Boolean Save_Update_or_Delete()
            {
                ...
                Result<int> cuResult = _WBMDB.ProcessingRule_Update(saveRow).GetAwaiter().GetResult();
                ...
                if (cuResult.IsSuccess)
                {
                    ...
                    if (chResult.IsSuccess)
                    {
                        ...
                        int ChangeHeaderTableID = (int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.ProcessingRule;
