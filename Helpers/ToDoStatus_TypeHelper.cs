...
{
    public class ToDoStatus_TypeHelper
    {
        private static Logger logger = LogManager.Setup().LoadConfigurationFromAppSettings().GetCurrentClassLogger();
        public static async Task<Result<List<wbm_common.DataObjects.ToDoStatus_Type.dbRow>>> ExecuteSearch(wbm_common.DataObjects.ToDoStatus_Type.Search _ToDoStatus_TypeSearchData, WBMDatabase _WBMDB, IHubCallerClients Clients = null, Func<IHubCallerClients, WBMDatabase, string, int, int, wbm_common.DataObjects.LongRunProcessHeader.dbRow, Task<string>> inCallBackStatusFunction = null, wbm_common.DataObjects.LongRunProcessHeader.dbRow longRunProcessHeader = null)
        {
            Result<List<wbm_common.DataObjects.ToDoStatus_Type.dbRow>> rc;
            List<wbm_common.DataObjects.ToDoStatus_Type.dbRow> ToDoStatus_Types;
            switch (_ToDoStatus_TypeSearchData.SearchMode)
            {
                case wbm_common.DataObjects.ToDoStatus_Type.Search.SearchModeType.Undefined:
                    {
                        _ToDoStatus_TypeSearchData.GenerateSqlQuery();
                        rc = await _WBMDB.ToDoStatus_Type_ListBySql(_ToDoStatus_TypeSearchData.SqlQuery, _ToDoStatus_TypeSearchData.ListSqlParameter);
                        break;
                    }
                case wbm_common.DataObjects.ToDoStatus_Type.Search.SearchModeType.SingleID:
                    {
                        rc = await _WBMDB.ToDoStatus_TypeList_SearchByID(_ToDoStatus_TypeSearchData.ID);
                        break;
                    }
                case wbm_common.DataObjects.ToDoStatus_Type.Search.SearchModeType.AllRecords:
                    {
                        rc = await _WBMDB.ToDoStatus_Type_List();
                        break;
                    }
                default:
                    {
                        logger.Error("ToDoStatus_TypeHelper.ExecuteSearch: ToDoStatus_Type Search Mode Type Error");
                        rc = Result.Exception<List<wbm_common.DataObjects.ToDoStatus_Type.dbRow>>("Internal Exception occured");
                        break;
                    }
            }
        ...
        public class ToDoStatus_TypeSave
        {
            private wbm_common.DataObjects.ToDoStatus_Type saveObject { get; set; }
            private wbm_common.DataObjects.ToDoStatus_Type.dbRow saveRow { get; set; }

            private wbm_common.DataObjects.ToDoStatus_Type.dbRow existRow { get; set; }
            ...
            public ToDoStatus_TypeSave(wbm_common.DataObjects.ToDoStatus_Type saveObject, int UserID, WBMDatabase wBMDB)
            {
            ...
            public Boolean CheckExist()
            {
                ...
                if (saveRow.ID > 0)
                {
                    Result<wbm_common.DataObjects.ToDoStatus_Type.dbRow> rc = _WBMDB.ToDoStatus_Type_ReadByID(saveRow.ID).GetAwaiter().GetResult();
                ...
                {
                    Result<wbm_common.DataObjects.ToDoStatus_Type.dbRow> rcexist = _WBMDB.ToDoStatus_Type_SearchByID(saveRow.ID).GetAwaiter().GetResult();
                    ...
                    else
                    {
                        ...
                        logger.Error("ToDoStatus_TypeSave ID " + saveRow.ID.ToString() + " already exists");
                        ReturnState = 400;
                        SavedSuccessfully = false;
                        return false;
                    }
            ...
            public Boolean CheckForChanges()
            {
                ...
                return ct.CheckForChanges(existRow, saveRow, wbm_common.DataObjects.ToDoStatus_Type.ListChangeTrackerData);
            }
            ...
            public void SetChanged()
            {
                ...
                else
                {
                    ...
                    changeHeaderRow = WBM_API.Helpers.ChangeTracking.Create_ChangeHeader_dbRow((int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.ToDoStatus_Type,
            ...
            {
                Result<int> sr = _WBMDB.ToDoStatus_Type_Insert(saveRow).GetAwaiter().GetResult();
            ...
            public Boolean Save_Update_or_Delete()
            {
                ...
                Result<int> cuResult = _WBMDB.ToDoStatus_Type_Update(saveRow).GetAwaiter().GetResult();
                ...
                if (cuResult.IsSuccess)
                {
                    ...
                    if (chResult.IsSuccess)
                    {
                        ...
                        int ChangeHeaderTableID = (int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.ToDoStatus_Type;
