...
{
    public class PriorityOrderHelper
    {
        private static Logger logger = LogManager.Setup().LoadConfigurationFromAppSettings().GetCurrentClassLogger();
        public static async Task<Result<List<wbm_common.DataObjects.PriorityOrder.dbRow>>> ExecuteSearch(wbm_common.DataObjects.PriorityOrder.Search _PriorityOrderSearchData, WBMDatabase _WBMDB, IHubCallerClients Clients = null, Func<IHubCallerClients, WBMDatabase, string, int, int, wbm_common.DataObjects.LongRunProcessHeader.dbRow, Task<string>> inCallBackStatusFunction = null, wbm_common.DataObjects.LongRunProcessHeader.dbRow longRunProcessHeader = null)
        {
            Result<List<wbm_common.DataObjects.PriorityOrder.dbRow>> rc;
            List<wbm_common.DataObjects.PriorityOrder.dbRow> PriorityOrders;
            switch (_PriorityOrderSearchData.SearchMode)
            {
                case wbm_common.DataObjects.PriorityOrder.Search.SearchModeType.Undefined:
                    {
                        _PriorityOrderSearchData.GenerateSqlQuery();
                        rc = await _WBMDB.PriorityOrder_ListBySql(_PriorityOrderSearchData.SqlQuery, _PriorityOrderSearchData.ListSqlParameter);
                        break;
                    }
                case wbm_common.DataObjects.PriorityOrder.Search.SearchModeType.SingleID:
                    {
                        rc = await _WBMDB.PriorityOrderList_SearchByID(_PriorityOrderSearchData.ID);
                        break;
                    }
                case wbm_common.DataObjects.PriorityOrder.Search.SearchModeType.AllRecords:
                    {
                        rc = await _WBMDB.PriorityOrder_List();
                        break;
                    }
                case wbm_common.DataObjects.PriorityOrder.Search.SearchModeType.SingleSite:
                    {
                        if (_PriorityOrderSearchData.ListSiteID == null || _PriorityOrderSearchData.ListSiteID.Count() != 1)
                        {
                            logger.Error("PriorityOrderHelper.ExecuteSearch: PriorityOrder SingleSite doesn't have single site input");
                            rc = Result.Exception<List<wbm_common.DataObjects.PriorityOrder.dbRow>>("Internal Exception occured");
                            break;
                        }
                        rc = await _WBMDB.PriorityOrder_ListBySingleSiteID(_PriorityOrderSearchData.ListSiteID[0]);
                        break;
                    }
                default:
                    {
                        logger.Error("PriorityOrderHelper.ExecuteSearch: PriorityOrder Search Mode Type Error");
                        rc = Result.Exception<List<wbm_common.DataObjects.PriorityOrder.dbRow>>("Internal Exception occured");
                        break;
                    }
            }
        ...
        public class PriorityOrderSave
        {
            private wbm_common.DataObjects.PriorityOrder saveObject { get; set; }
            private wbm_common.DataObjects.PriorityOrder.dbRow saveRow { get; set; }

            private wbm_common.DataObjects.PriorityOrder.dbRow existRow { get; set; }
            ...
            public PriorityOrderSave(wbm_common.DataObjects.PriorityOrder saveObject, int UserID, WBMDatabase wBMDB)
            {
            ...
            public Boolean CheckExist()
            {
                ...
                if (saveRow.ID > 0)
                {
                    Result<wbm_common.DataObjects.PriorityOrder.dbRow> rc = _WBMDB.PriorityOrder_ReadByID(saveRow.ID).GetAwaiter().GetResult();
                ...
                {
                    Result<wbm_common.DataObjects.PriorityOrder.dbRow> rcexist = _WBMDB.PriorityOrder_SearchByID(saveRow.ID).GetAwaiter().GetResult();
                    ...
                    else
                    {
                        ...
                        logger.Error("PriorityOrderSave ID " + saveRow.ID + " already exists");
                        ReturnState = 400;
                        SavedSuccessfully = false;
                        return false;
                    }
            ...
            public Boolean CheckForChanges()
            {
                ...
                return ct.CheckForChanges(existRow, saveRow, wbm_common.DataObjects.PriorityOrder.ListChangeTrackerData);
            }
            ...
            public void SetChanged()
            {
                ...
                else
                {
                    ...
                    changeHeaderRow = WBM_API.Helpers.ChangeTracking.Create_ChangeHeader_dbRow((int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.PriorityOrder,
            ...
            {
                Result<int> sr = _WBMDB.PriorityOrder_Insert(saveRow).GetAwaiter().GetResult();
            ...
            public Boolean Save_Update_or_Delete()
            {
                ...
                Result<int> cuResult = _WBMDB.PriorityOrder_Update(saveRow).GetAwaiter().GetResult();
                ...
                if (cuResult.IsSuccess)
                {
                    ...
                    if (chResult.IsSuccess)
                    {
                        ...
                        int ChangeHeaderTableID = (int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.PriorityOrder;
