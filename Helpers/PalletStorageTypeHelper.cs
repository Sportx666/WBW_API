...
{
    public class PalletStorageTypeHelper
    {
        private static Logger logger = LogManager.Setup().LoadConfigurationFromAppSettings().GetCurrentClassLogger();
        public static async Task<Result<List<wbm_common.DataObjects.PalletStorageType.dbRow>>> ExecuteSearch(wbm_common.DataObjects.PalletStorageType.Search _PalletStorageTypeSearchData, WBMDatabase _WBMDB, IHubCallerClients Clients = null, Func<IHubCallerClients, WBMDatabase, string, int, int, wbm_common.DataObjects.LongRunProcessHeader.dbRow, Task<string>> inCallBackStatusFunction = null, wbm_common.DataObjects.LongRunProcessHeader.dbRow longRunProcessHeader = null)
        {
            Result<List<wbm_common.DataObjects.PalletStorageType.dbRow>> rc;
            List<wbm_common.DataObjects.PalletStorageType.dbRow> PalletStorageTypes;
            switch (_PalletStorageTypeSearchData.SearchMode)
            {
                case wbm_common.DataObjects.PalletStorageType.Search.SearchModeType.Undefined:
                    {
                        _PalletStorageTypeSearchData.GenerateSqlQuery();
                        rc = await _WBMDB.PalletStorageType_ListBySql(_PalletStorageTypeSearchData.SqlQuery, _PalletStorageTypeSearchData.ListSqlParameter);
                        break;
                    }
                case wbm_common.DataObjects.PalletStorageType.Search.SearchModeType.SingleID:
                    {
                        rc = await _WBMDB.PalletStorageTypeList_SearchByID(_PalletStorageTypeSearchData.ID);
                        break;
                    }
                case wbm_common.DataObjects.PalletStorageType.Search.SearchModeType.AllRecords:
                    {
                        rc = await _WBMDB.PalletStorageType_List();
                        break;
                    }
                default:
                    {
                        logger.Error("PalletStorageTypeHelper.ExecuteSearch: PalletStorageType Search Mode Type Error");
                        rc = Result.Exception<List<wbm_common.DataObjects.PalletStorageType.dbRow>>("Internal Exception occured");
                        break;
                    }
            }
        ...
        public class PalletStorageTypeSave
        {
            private wbm_common.DataObjects.PalletStorageType saveObject { get; set; }
            private wbm_common.DataObjects.PalletStorageType.dbRow saveRow { get; set; }

            private wbm_common.DataObjects.PalletStorageType.dbRow existRow { get; set; }
            ...
            public PalletStorageTypeSave(wbm_common.DataObjects.PalletStorageType saveObject, int UserID, WBMDatabase wBMDB)
            {
            ...
            public Boolean CheckExist()
            {
                ...
                if (saveRow.ID > 0)
                {
                    Result<wbm_common.DataObjects.PalletStorageType.dbRow> rc = _WBMDB.PalletStorageType_ReadByID(saveRow.ID).GetAwaiter().GetResult();
                ...
                {
                    Result<wbm_common.DataObjects.PalletStorageType.dbRow> rcexist = _WBMDB.PalletStorageType_SearchByID(saveRow.ID).GetAwaiter().GetResult();
                    ...
                    else
                    {
                        ...
                        logger.Error("PalletStorageTypeSave ID " + saveRow.ID.ToString() + " already exists");
                        ReturnState = 400;
                        SavedSuccessfully = false;
                        return false;
                    }
            ...
            public Boolean CheckForChanges()
            {
                ...
                return ct.CheckForChanges(existRow, saveRow, wbm_common.DataObjects.PalletStorageType.ListChangeTrackerData);
            }
            ...
            public void SetChanged()
            {
                ...
                else
                {
                    ...
                    changeHeaderRow = WBM_API.Helpers.ChangeTracking.Create_ChangeHeader_dbRow((int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.PalletStorageType,
            ...
            {
                Result<int> sr = _WBMDB.PalletStorageType_Insert(saveRow).GetAwaiter().GetResult();
            ...
            public Boolean Save_Update_or_Delete()
            {
                ...
                Result<int> cuResult = _WBMDB.PalletStorageType_Update(saveRow).GetAwaiter().GetResult();
                ...
                if (cuResult.IsSuccess)
                {
                    ...
                    if (chResult.IsSuccess)
                    {
                        ...
                        int ChangeHeaderTableID = (int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.PalletStorageType;
