...
{
    public class StorageRateCodeFromHelper
    {
        private static Logger logger = LogManager.Setup().LoadConfigurationFromAppSettings().GetCurrentClassLogger();
        public static async Task<Result<List<wbm_common.DataObjects.StorageRateCodeFrom.dbRow>>> ExecuteSearch(wbm_common.DataObjects.StorageRateCodeFrom.Search _StorageRateCodeFromSearchData, WBMDatabase _WBMDB, IHubCallerClients Clients = null, Func<IHubCallerClients, WBMDatabase, string, int, int, wbm_common.DataObjects.LongRunProcessHeader.dbRow, Task<string>> inCallBackStatusFunction = null, wbm_common.DataObjects.LongRunProcessHeader.dbRow longRunProcessHeader = null)
        {
            Result<List<wbm_common.DataObjects.StorageRateCodeFrom.dbRow>> rc;
            List<wbm_common.DataObjects.StorageRateCodeFrom.dbRow> StorageRateCodeFroms;
            switch (_StorageRateCodeFromSearchData.SearchMode)
            {
                case wbm_common.DataObjects.StorageRateCodeFrom.Search.SearchModeType.Undefined:
                    {
                        _StorageRateCodeFromSearchData.GenerateSqlQuery();
                        rc = await _WBMDB.StorageRateCodeFrom_ListBySql(_StorageRateCodeFromSearchData.SqlQuery, _StorageRateCodeFromSearchData.ListSqlParameter);
                        break;
                    }
                case wbm_common.DataObjects.StorageRateCodeFrom.Search.SearchModeType.SingleID:
                    {
                        rc = await _WBMDB.StorageRateCodeFromList_SearchByID(_StorageRateCodeFromSearchData.ID);
                        break;
                    }
                case wbm_common.DataObjects.StorageRateCodeFrom.Search.SearchModeType.AllRecords:
                    {
                        rc = await _WBMDB.StorageRateCodeFrom_List();
                        break;
                    }
                default:
                    {
                        logger.Error("StorageRateCodeFromHelper.ExecuteSearch: StorageRateCodeFrom Search Mode Type Error");
                        rc = Result.Exception<List<wbm_common.DataObjects.StorageRateCodeFrom.dbRow>>("Internal Exception occured");
                        break;
                    }
            }
        ...
        public class StorageRateCodeFromSave
        {
            private wbm_common.DataObjects.StorageRateCodeFrom saveObject { get; set; }
            private wbm_common.DataObjects.StorageRateCodeFrom.dbRow saveRow { get; set; }

            private wbm_common.DataObjects.StorageRateCodeFrom.dbRow existRow { get; set; }
            ...
            public StorageRateCodeFromSave(wbm_common.DataObjects.StorageRateCodeFrom saveObject, int UserID, WBMDatabase wBMDB)
            {
            ...
            public Boolean CheckExist()
            {
                ...
                if (saveRow.ID > 0)
                {
                    Result<wbm_common.DataObjects.StorageRateCodeFrom.dbRow> rc = _WBMDB.StorageRateCodeFrom_ReadByID(saveRow.ID).GetAwaiter().GetResult();
                ...
                {
                    Result<wbm_common.DataObjects.StorageRateCodeFrom.dbRow> rcexist = _WBMDB.StorageRateCodeFrom_SearchByID(saveRow.ID).GetAwaiter().GetResult();
                    ...
                    else
                    {
                        ...
                        logger.Error("StorageRateCodeFromSave ID " + saveRow.ID.ToString() + " already exists");
                        ReturnState = 400;
                        SavedSuccessfully = false;
                        return false;
                    }
            ...
            public Boolean CheckForChanges()
            {
                ...
                return ct.CheckForChanges(existRow, saveRow, wbm_common.DataObjects.StorageRateCodeFrom.ListChangeTrackerData);
            }
            ...
            public void SetChanged()
            {
                ...
                else
                {
                    ...
                    changeHeaderRow = WBM_API.Helpers.ChangeTracking.Create_ChangeHeader_dbRow((int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.StorageRateCodeFrom,
            ...
            {
                Result<int> sr = _WBMDB.StorageRateCodeFrom_Insert(saveRow).GetAwaiter().GetResult();
            ...
            public Boolean Save_Update_or_Delete()
            {
                ...
                Result<int> cuResult = _WBMDB.StorageRateCodeFrom_Update(saveRow).GetAwaiter().GetResult();
                ...
                if (cuResult.IsSuccess)
                {
                    ...
                    if (chResult.IsSuccess)
                    {
                        ...
                        int ChangeHeaderTableID = (int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.StorageRateCodeFrom;
