...
{
    public class InvoiceCycleCurrentHelper
    {
        private static Logger logger = LogManager.Setup().LoadConfigurationFromAppSettings().GetCurrentClassLogger();
        public static async Task<Result<List<wbm_common.DataObjects.InvoiceCycleCurrent.dbRow>>> ExecuteSearch(wbm_common.DataObjects.InvoiceCycleCurrent.Search _InvoiceCycleCurrentSearchData, WBMDatabase _WBMDB, IHubCallerClients Clients = null, Func<IHubCallerClients, WBMDatabase, string, int, int, wbm_common.DataObjects.LongRunProcessHeader.dbRow, Task<string>> inCallBackStatusFunction = null, wbm_common.DataObjects.LongRunProcessHeader.dbRow longRunProcessHeader = null)
        {
            Result<List<wbm_common.DataObjects.InvoiceCycleCurrent.dbRow>> rc;
            List<wbm_common.DataObjects.InvoiceCycleCurrent.dbRow> InvoiceCycleCurrents;
            switch (_InvoiceCycleCurrentSearchData.SearchMode)
            {
                case wbm_common.DataObjects.InvoiceCycleCurrent.Search.SearchModeType.Undefined:
                    {
                        _InvoiceCycleCurrentSearchData.GenerateSqlQuery();
                        rc = await _WBMDB.InvoiceCycleCurrent_ListBySql(_InvoiceCycleCurrentSearchData.SqlQuery, _InvoiceCycleCurrentSearchData.ListSqlParameter);
                        break;
                    }
                case wbm_common.DataObjects.InvoiceCycleCurrent.Search.SearchModeType.SingleID:
                    {
                        rc = await _WBMDB.InvoiceCycleCurrentList_SearchByID(_InvoiceCycleCurrentSearchData.ID);
                        break;
                    }
                case wbm_common.DataObjects.InvoiceCycleCurrent.Search.SearchModeType.AllRecords:
                    {
                        rc = await _WBMDB.InvoiceCycleCurrent_List();
                        break;
                    }
                default:
                    {
                        logger.Error("InvoiceCycleCurrentHelper.ExecuteSearch: InvoiceCycleCurrent Search Mode Type Error");
                        rc = Result.Exception<List<wbm_common.DataObjects.InvoiceCycleCurrent.dbRow>>("Internal Exception occured");
                        break;
                    }
            }
        ...
        public class InvoiceCycleCurrentSave
        {
            private wbm_common.DataObjects.InvoiceCycleCurrent saveObject { get; set; }
            private wbm_common.DataObjects.InvoiceCycleCurrent.dbRow saveRow { get; set; }

            private wbm_common.DataObjects.InvoiceCycleCurrent.dbRow existRow { get; set; }
            ...
            public InvoiceCycleCurrentSave(wbm_common.DataObjects.InvoiceCycleCurrent saveObject, int UserID, WBMDatabase wBMDB)
            {
            ...
            public Boolean CheckExist()
            {
                ...
                if (saveRow.ID > 0)
                {
                    Result<wbm_common.DataObjects.InvoiceCycleCurrent.dbRow> rc = _WBMDB.InvoiceCycleCurrent_ReadByID(saveRow.ID).GetAwaiter().GetResult();
                ...
                {
                    Result<wbm_common.DataObjects.InvoiceCycleCurrent.dbRow> rcexist = _WBMDB.InvoiceCycleCurrent_SearchByID(saveRow.ID).GetAwaiter().GetResult();
                    ...
                    else
                    {
                        ...
                        logger.Error("InvoiceCycleCurrentSave ID " + saveRow.ID.ToString() + " already exists");
                        ReturnState = 400;
                        SavedSuccessfully = false;
                        return false;
                    }
            ...
            public Boolean CheckForChanges()
            {
                ...
                return ct.CheckForChanges(existRow, saveRow, wbm_common.DataObjects.InvoiceCycleCurrent.ListChangeTrackerData);
            }
            ...
            public void SetChanged()
            {
                ...
                else
                {
                    ...
                    changeHeaderRow = WBM_API.Helpers.ChangeTracking.Create_ChangeHeader_dbRow((int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.InvoiceCycleCurrent,
            ...
            {
                Result<int> sr = _WBMDB.InvoiceCycleCurrent_Insert(saveRow).GetAwaiter().GetResult();
            ...
            public Boolean Save_Update_or_Delete()
            {
                ...
                Result<int> cuResult = _WBMDB.InvoiceCycleCurrent_Update(saveRow).GetAwaiter().GetResult();
                ...
                if (cuResult.IsSuccess)
                {
                    ...
                    if (chResult.IsSuccess)
                    {
                        ...
                        int ChangeHeaderTableID = (int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.InvoiceCycleCurrent;
