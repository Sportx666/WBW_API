...
{
    public class ContainerHeavyLiftHelper
    {
        private static Logger logger = LogManager.Setup().LoadConfigurationFromAppSettings().GetCurrentClassLogger();
        public static async Task<Result<List<wbm_common.DataObjects.ContainerHeavyLift.dbRow>>> ExecuteSearch(wbm_common.DataObjects.ContainerHeavyLift.Search _ContainerHeavyLiftSearchData, WBMDatabase _WBMDB, IHubCallerClients Clients = null, Func<IHubCallerClients, WBMDatabase, string, int, int, wbm_common.DataObjects.LongRunProcessHeader.dbRow, Task<string>> inCallBackStatusFunction = null, wbm_common.DataObjects.LongRunProcessHeader.dbRow longRunProcessHeader = null)
        {
            Result<List<wbm_common.DataObjects.ContainerHeavyLift.dbRow>> rc;
            List<wbm_common.DataObjects.ContainerHeavyLift.dbRow> ContainerHeavyLifts;
            switch (_ContainerHeavyLiftSearchData.SearchMode)
            {
                case wbm_common.DataObjects.ContainerHeavyLift.Search.SearchModeType.Undefined:
                    {
                        _ContainerHeavyLiftSearchData.GenerateSqlQuery();
                        rc = await _WBMDB.ContainerHeavyLift_ListBySql(_ContainerHeavyLiftSearchData.SqlQuery, _ContainerHeavyLiftSearchData.ListSqlParameter);
                        break;
                    }
                case wbm_common.DataObjects.ContainerHeavyLift.Search.SearchModeType.SingleID:
                    {
                        rc = await _WBMDB.ContainerHeavyLiftList_SearchByID(_ContainerHeavyLiftSearchData.ID);
                        break;
                    }
                case wbm_common.DataObjects.ContainerHeavyLift.Search.SearchModeType.AllRecords:
                    {
                        rc = await _WBMDB.ContainerHeavyLift_List();
                        break;
                    }
                case wbm_common.DataObjects.ContainerHeavyLift.Search.SearchModeType.SingleOwnerID:
                    {
                        if (_ContainerHeavyLiftSearchData.ListOwnerID == null || _ContainerHeavyLiftSearchData.ListOwnerID.Count() != 1)
                        {
                            logger.Error("ContainerHeavyLiftHelper.ExecuteSearch: ActionRateCode SingleOwner doesn't have single owner input");
                            rc = Result.Exception<List<wbm_common.DataObjects.ContainerHeavyLift.dbRow>>("Internal Exception occured");
                            break;
                        }
                        rc = await _WBMDB.ContainerHeavyLift_ListByOwnerID(_ContainerHeavyLiftSearchData.ListOwnerID[0]);
                        break;
                    }
                default:
                    {
                        logger.Error("ContainerHeavyLiftHelper.ExecuteSearch: ContainerHeavyLift Search Mode Type Error");
                        rc = Result.Exception<List<wbm_common.DataObjects.ContainerHeavyLift.dbRow>>("Internal Exception occured");
                        break;
                    }
            }
        ...
        public class ContainerHeavyLiftSave
        {
            private wbm_common.DataObjects.ContainerHeavyLift saveObject { get; set; }
            private wbm_common.DataObjects.ContainerHeavyLift.dbRow saveRow { get; set; }

            private wbm_common.DataObjects.ContainerHeavyLift.dbRow existRow { get; set; }
            ...
            public ContainerHeavyLiftSave(wbm_common.DataObjects.ContainerHeavyLift saveObject, int UserID, WBMDatabase wBMDB)
            {
            ...
            public Boolean CheckExist()
            {
                ...
                if (saveRow.ID > 0)
                {
                    Result<wbm_common.DataObjects.ContainerHeavyLift.dbRow> rc = _WBMDB.ContainerHeavyLift_ReadByID(saveRow.ID).GetAwaiter().GetResult();
                ...
                {
                    Result<wbm_common.DataObjects.ContainerHeavyLift.dbRow> rcexist = _WBMDB.ContainerHeavyLift_SearchByID(saveRow.ID).GetAwaiter().GetResult();
                    ...
                    else
                    {
                        ...
                        logger.Error("ContainerHeavyLiftSave ID " + saveRow.ID.ToString() + " already exists");
                        ReturnState = 400;
                        SavedSuccessfully = false;
                        return false;
                    }
            ...
            public Boolean CheckForChanges()
            {
                ...
                return ct.CheckForChanges(existRow, saveRow, wbm_common.DataObjects.ContainerHeavyLift.ListChangeTrackerData);
            }
            ...
            public void SetChanged()
            {
                ...
                else
                {
                    ...
                    changeHeaderRow = WBM_API.Helpers.ChangeTracking.Create_ChangeHeader_dbRow((int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.ContainerHeavyLift,
            ...
            {
                Result<int> sr = _WBMDB.ContainerHeavyLift_Insert(saveRow).GetAwaiter().GetResult();
            ...
            public Boolean Save_Update_or_Delete()
            {
                ...
                Result<int> cuResult = _WBMDB.ContainerHeavyLift_Update(saveRow).GetAwaiter().GetResult();
                ...
                if (cuResult.IsSuccess)
                {
                    ...
                    if (chResult.IsSuccess)
                    {
                        ...
                        int ChangeHeaderTableID = (int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.ContainerHeavyLift;
