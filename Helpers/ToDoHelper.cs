...
namespace WBM_API.Helpers
{
    public class ToDoHelper
    {
        ...
        public static async Task<Result<List<wbm_common.DataObjects.ToDo.dbRow>>> ExecuteSearch(wbm_common.DataObjects.ToDo.Search _ToDoSearchData, WBMDatabase _WBMDB, IHubCallerClients Clients = null, Func<IHubCallerClients, WBMDatabase, string, int, int, wbm_common.DataObjects.LongRunProcessHeader.dbRow, Task<string>> inCallBackStatusFunction = null, wbm_common.DataObjects.LongRunProcessHeader.dbRow longRunProcessHeader = null)
        {
            Result<List<wbm_common.DataObjects.ToDo.dbRow>> rc;
            List<wbm_common.DataObjects.ToDo.dbRow> ToDos;
            switch (_ToDoSearchData.SearchMode)
            {
                case wbm_common.DataObjects.ToDo.Search.SearchModeType.Undefined:
                    {
                        _ToDoSearchData.GenerateSqlQuery();
                        rc = await _WBMDB.ToDo_ListBySql(_ToDoSearchData.SqlQuery, _ToDoSearchData.ListSqlParameter);
                        break;
                    }
                case wbm_common.DataObjects.ToDo.Search.SearchModeType.SingleID:
                    {
                        rc = await _WBMDB.ToDoList_SearchByID(_ToDoSearchData.ID);
                        break;
                    }
                default:
                    {
                        logger.Error("ToDoHelper.ExecuteSearch: ToDo Search Mode Type Error");
                        rc = Result.Exception<List<wbm_common.DataObjects.ToDo.dbRow>>("Internal Exception occured");
                        break;
                    }
            }
        ...
        {
            private wbm_common.DataObjects.ToDo saveObject { get; set; }
            private wbm_common.DataObjects.ToDo.dbRow saveRow { get; set; }

            private wbm_common.DataObjects.ToDo.dbRow existRow { get; set; }
            ...
            public ToDoSave(wbm_common.DataObjects.ToDo saveObject, int UserID, WBMDatabase wBMDB)
            {
            ...
            public Boolean CheckExist()
            {
                ...
                if (saveRow.ID > 0)
                {
                    Result<wbm_common.DataObjects.ToDo.dbRow> rc = _WBMDB.ToDo_ReadByID(saveRow.ID).GetAwaiter().GetResult();
                ...
                {
                    Result<wbm_common.DataObjects.ToDo.dbRow> rcexist = _WBMDB.ToDo_SearchByID(saveRow.ID).GetAwaiter().GetResult();
                    ...
                    else
                    {
                        ...
                        logger.Error("ToDoSave ID " + saveRow.ID.ToString() + " already exists");
                        ReturnState = 400;
                        SavedSuccessfully = false;
                        return false;
                    }
            ...
            public Boolean CheckForChanges()
            {
                ...
                return ct.CheckForChanges(existRow, saveRow, wbm_common.DataObjects.ToDo.ListChangeTrackerData);
            }
            ...
            public void SetChanged()
            {
                ...
                else
                {
                    ...
                    changeHeaderRow = WBM_API.Helpers.ChangeTracking.Create_ChangeHeader_dbRow((int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.ToDo,
            ...
            {
                Result<int> sr = _WBMDB.ToDo_Insert(saveRow).GetAwaiter().GetResult();
            ...
            public Boolean Save_Update_or_Delete()
            {
                ...
                Result<int> cuResult = _WBMDB.ToDo_Update(saveRow).GetAwaiter().GetResult();
                ...
                if (cuResult.IsSuccess)
                {
                    ...
                    if (chResult.IsSuccess)
                    {
                        ...
                        int ChangeHeaderTableID = (int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.ToDo;
