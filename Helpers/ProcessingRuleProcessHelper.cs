...
{
    public class ProcessingRuleProcessHelper
    {
        private static Logger logger = LogManager.Setup().LoadConfigurationFromAppSettings().GetCurrentClassLogger();
        public static async Task<Result<List<wbm_common.DataObjects.ProcessingRuleProcess.dbRow>>> ExecuteSearch(wbm_common.DataObjects.ProcessingRuleProcess.Search _ProcessingRuleProcessSearchData, WBMDatabase _WBMDB, IHubCallerClients Clients = null, Func<IHubCallerClients, WBMDatabase, string, int, int, wbm_common.DataObjects.LongRunProcessHeader.dbRow, Task<string>> inCallBackStatusFunction = null, wbm_common.DataObjects.LongRunProcessHeader.dbRow longRunProcessHeader = null)
        {
            Result<List<wbm_common.DataObjects.ProcessingRuleProcess.dbRow>> rc;
            List<wbm_common.DataObjects.ProcessingRuleProcess.dbRow> ProcessingRuleProcesss;
            switch (_ProcessingRuleProcessSearchData.SearchMode)
            {
                case wbm_common.DataObjects.ProcessingRuleProcess.Search.SearchModeType.Undefined:
                    {
                        _ProcessingRuleProcessSearchData.GenerateSqlQuery();
                        rc = await _WBMDB.ProcessingRuleProcess_ListBySql(_ProcessingRuleProcessSearchData.SqlQuery, _ProcessingRuleProcessSearchData.ListSqlParameter);
                        break;
                    }
                case wbm_common.DataObjects.ProcessingRuleProcess.Search.SearchModeType.SingleID:
                    {
                        rc = await _WBMDB.ProcessingRuleProcessList_SearchByID(_ProcessingRuleProcessSearchData.ID);
                        break;
                    }
                case wbm_common.DataObjects.ProcessingRuleProcess.Search.SearchModeType.AllRecords:
                    {
                        rc = await _WBMDB.ProcessingRuleProcess_List();
                        break;
                    }
                default:
                    {
                        logger.Error("ProcessingRuleProcessHelper.ExecuteSearch: ProcessingRuleProcess Search Mode Type Error");
                        rc = Result.Exception<List<wbm_common.DataObjects.ProcessingRuleProcess.dbRow>>("Internal Exception occured");
                        break;
                    }
            }
        ...
        public class ProcessingRuleProcessSave
        {
            private wbm_common.DataObjects.ProcessingRuleProcess saveObject { get; set; }
            private wbm_common.DataObjects.ProcessingRuleProcess.dbRow saveRow { get; set; }

            private wbm_common.DataObjects.ProcessingRuleProcess.dbRow existRow { get; set; }
            ...
            public ProcessingRuleProcessSave(wbm_common.DataObjects.ProcessingRuleProcess saveObject, int UserID, WBMDatabase wBMDB)
            {
            ...
            public Boolean CheckExist()
            {
                ...
                if (saveRow.ID > 0)
                {
                    Result<wbm_common.DataObjects.ProcessingRuleProcess.dbRow> rc = _WBMDB.ProcessingRuleProcess_ReadByID(saveRow.ID).GetAwaiter().GetResult();
                ...
                {
                    Result<wbm_common.DataObjects.ProcessingRuleProcess.dbRow> rcexist = _WBMDB.ProcessingRuleProcess_SearchByID(saveRow.ID).GetAwaiter().GetResult();
                    ...
                    else
                    {
                        ...
                        logger.Error("ProcessingRuleProcessSave ID " + saveRow.ID.ToString() + " already exists");
                        ReturnState = 400;
                        SavedSuccessfully = false;
                        return false;
                    }
            ...
            public Boolean CheckForChanges()
            {
                ...
                return ct.CheckForChanges(existRow, saveRow, wbm_common.DataObjects.ProcessingRuleProcess.ListChangeTrackerData);
            }
            ...
            public void SetChanged()
            {
                ...
                else
                {
                    ...
                    changeHeaderRow = WBM_API.Helpers.ChangeTracking.Create_ChangeHeader_dbRow((int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.ProcessingRuleProcess,
            ...
            {
                Result<int> sr = _WBMDB.ProcessingRuleProcess_Insert(saveRow).GetAwaiter().GetResult();
            ...
            public Boolean Save_Update_or_Delete()
            {
                ...
                Result<int> cuResult = _WBMDB.ProcessingRuleProcess_Update(saveRow).GetAwaiter().GetResult();
                ...
                if (cuResult.IsSuccess)
                {
                    ...
                    if (chResult.IsSuccess)
                    {
                        ...
                        int ChangeHeaderTableID = (int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.ProcessingRuleProcess;
