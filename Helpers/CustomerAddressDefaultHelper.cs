...
{
    public class CustomerAddressDefaultHelper
    {
        private static Logger logger = LogManager.Setup().LoadConfigurationFromAppSettings().GetCurrentClassLogger();
        public static async Task<Result<List<wbm_common.DataObjects.CustomerAddressDefault.dbRow>>> ExecuteSearch(wbm_common.DataObjects.CustomerAddressDefault.Search _CustomerAddressDefaultSearchData, WBMDatabase _WBMDB, IHubCallerClients Clients = null, Func<IHubCallerClients, WBMDatabase, string, int, int, wbm_common.DataObjects.LongRunProcessHeader.dbRow, Task<string>> inCallBackStatusFunction = null, wbm_common.DataObjects.LongRunProcessHeader.dbRow longRunProcessHeader = null)
        {
            Result<List<wbm_common.DataObjects.CustomerAddressDefault.dbRow>> rc;
            List<wbm_common.DataObjects.CustomerAddressDefault.dbRow> CustomerAddressDefaults;
            switch (_CustomerAddressDefaultSearchData.SearchMode)
            {
                case wbm_common.DataObjects.CustomerAddressDefault.Search.SearchModeType.Undefined:
                    {
                        _CustomerAddressDefaultSearchData.GenerateSqlQuery();
                        rc = await _WBMDB.CustomerAddressDefault_ListBySql(_CustomerAddressDefaultSearchData.SqlQuery, _CustomerAddressDefaultSearchData.ListSqlParameter);
                        break;
                    }
                case wbm_common.DataObjects.CustomerAddressDefault.Search.SearchModeType.SingleID:
                    {
                        rc = await _WBMDB.CustomerAddressDefault_SearchByID(_CustomerAddressDefaultSearchData.ID);
                        break;
                    }
                case wbm_common.DataObjects.CustomerAddressDefault.Search.SearchModeType.AllRecords:
                    {
                        rc = await _WBMDB.CustomerAddressDefault_List();
                        break;
                    }
                default:
                    {
                        logger.Error("CustomerAddressDefaultHelper.ExecuteSearch: CustomerAddressDefault Search Mode Type Error");
                        rc = Result.Exception<List<wbm_common.DataObjects.CustomerAddressDefault.dbRow>>("Internal Exception occured");
                        break;
                    }
            }
        ...
        public class CustomerAddressDefaultSave
        {
            private wbm_common.DataObjects.CustomerAddressDefault saveObject { get; set; }
            private wbm_common.DataObjects.CustomerAddressDefault.dbRow saveRow { get; set; }

            private wbm_common.DataObjects.CustomerAddressDefault.dbRow existRow { get; set; }
            ...
            public CustomerAddressDefaultSave(wbm_common.DataObjects.CustomerAddressDefault saveObject, int UserID, WBMDatabase wBMDB)
            {
            ...
            public Boolean CheckExist()
            {
                ...
                if (saveRow.ID > 0)
                {
                    Result<wbm_common.DataObjects.CustomerAddressDefault.dbRow> rc = _WBMDB.CustomerAddressDefault_ReadByID(saveRow.ID).GetAwaiter().GetResult();
                ...
                {
                    Result<wbm_common.DataObjects.CustomerAddressDefault.dbRow> rcexist = _WBMDB.CustomerAddressDefault_SearchByPublicID(saveRow.PublicID).GetAwaiter().GetResult();
                    ...
                    else
                    {
                        ...
                        logger.Error("CustomerAddressDefaultSave ID " + saveRow.ID.ToString() + " PublicID " + saveRow.PublicID + " already exists");
                        ReturnState = 400;
                        SavedSuccessfully = false;
                        return false;
                    }
            ...
            public Boolean CheckForChanges()
            {
                ...
                return ct.CheckForChanges(existRow, saveRow, wbm_common.DataObjects.CustomerAddressDefault.ListChangeTrackerData);
            }
            ...
            public void SetChanged()
            {
                ...
                else
                {
                    ...
                    changeHeaderRow = WBM_API.Helpers.ChangeTracking.Create_ChangeHeader_dbRow((int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.CustomerAddressDefault,
            ...
            {
                Result<int> sr = _WBMDB.CustomerAddressDefault_Insert(saveRow).GetAwaiter().GetResult();
            ...
            public Boolean Save_Update_or_Delete()
            {
                ...
                Result<int> cuResult = _WBMDB.CustomerAddressDefault_Update(saveRow).GetAwaiter().GetResult();
                ...
                if (cuResult.IsSuccess)
                {
                    ...
                    if (chResult.IsSuccess)
                    {
                        ...
                        int ChangeHeaderTableID = (int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.CustomerAddressDefault;
