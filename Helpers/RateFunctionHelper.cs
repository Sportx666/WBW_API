...
{
    public class RateFunctionHelper
    {
        private static Logger logger = LogManager.Setup().LoadConfigurationFromAppSettings().GetCurrentClassLogger();
        public static async Task<Result<List<wbm_common.DataObjects.RateFunction.dbRow>>> ExecuteSearch(wbm_common.DataObjects.RateFunction.Search _RateFunctionSearchData, WBMDatabase _WBMDB, IHubCallerClients Clients = null, Func<IHubCallerClients, WBMDatabase, string, int, int, wbm_common.DataObjects.LongRunProcessHeader.dbRow, Task<string>> inCallBackStatusFunction = null, wbm_common.DataObjects.LongRunProcessHeader.dbRow longRunProcessHeader = null)
        {
            Result<List<wbm_common.DataObjects.RateFunction.dbRow>> rc;
            List<wbm_common.DataObjects.RateFunction.dbRow> RateFunctions;
            switch (_RateFunctionSearchData.SearchMode)
            {
                case wbm_common.DataObjects.RateFunction.Search.SearchModeType.Undefined:
                    {
                        _RateFunctionSearchData.GenerateSqlQuery();
                        rc = await _WBMDB.RateFunction_ListBySql(_RateFunctionSearchData.SqlQuery, _RateFunctionSearchData.ListSqlParameter);
                        break;
                    }
                case wbm_common.DataObjects.RateFunction.Search.SearchModeType.SingleID:
                    {
                        rc = await _WBMDB.RateFunctionList_SearchByID(_RateFunctionSearchData.ID);
                        break;
                    }
                case wbm_common.DataObjects.RateFunction.Search.SearchModeType.AllRecords:
                    {
                        rc = await _WBMDB.RateFunction_List();
                        break;
                    }
                default:
                    {
                        logger.Error("RateFunctionHelper.ExecuteSearch: RateFunction Search Mode Type Error");
                        rc = Result.Exception<List<wbm_common.DataObjects.RateFunction.dbRow>>("Internal Exception occured");
                        break;
                    }
            }
        ...
        public class RateFunctionSave
        {
            private wbm_common.DataObjects.RateFunction saveObject { get; set; }
            private wbm_common.DataObjects.RateFunction.dbRow saveRow { get; set; }

            private wbm_common.DataObjects.RateFunction.dbRow existRow { get; set; }
            ...
            public RateFunctionSave(wbm_common.DataObjects.RateFunction saveObject, int UserID, WBMDatabase wBMDB)
            {
            ...
            public Boolean CheckExist()
            {
                ...
                if (saveRow.ID > 0)
                {
                    Result<wbm_common.DataObjects.RateFunction.dbRow> rc = _WBMDB.RateFunction_ReadByID(saveRow.ID).GetAwaiter().GetResult();
                ...
                {
                    Result<wbm_common.DataObjects.RateFunction.dbRow> rcexist = _WBMDB.RateFunction_SearchByID(saveRow.ID).GetAwaiter().GetResult();
                    ...
                    else
                    {
                        ...
                        logger.Error("RateFunctionSave ID " + saveRow.ID.ToString() + " already exists");
                        ReturnState = 400;
                        SavedSuccessfully = false;
                        return false;
                    }
            ...
            public Boolean CheckForChanges()
            {
                ...
                return ct.CheckForChanges(existRow, saveRow, wbm_common.DataObjects.RateFunction.ListChangeTrackerData);
            }
            ...
            public void SetChanged()
            {
                ...
                else
                {
                    ...
                    changeHeaderRow = WBM_API.Helpers.ChangeTracking.Create_ChangeHeader_dbRow((int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.RateFunction,
            ...
            {
                Result<int> sr = _WBMDB.RateFunction_Insert(saveRow).GetAwaiter().GetResult();
            ...
            public Boolean Save_Update_or_Delete()
            {
                ...
                Result<int> cuResult = _WBMDB.RateFunction_Update(saveRow).GetAwaiter().GetResult();
                ...
                if (cuResult.IsSuccess)
                {
                    ...
                    if (chResult.IsSuccess)
                    {
                        ...
                        int ChangeHeaderTableID = (int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.RateFunction;
