...
namespace WBM_API.Helpers
{
    public class SiteHelper
    {
        ...
        public static async Task<Result<List<wbm_common.DataObjects.Site.dbRow>>> ExecuteSearch(wbm_common.DataObjects.Site.Search _SiteSearchData, WBMDatabase _WBMDB, IHubCallerClients Clients = null, Func<IHubCallerClients, WBMDatabase, string, int, int, wbm_common.DataObjects.LongRunProcessHeader.dbRow, Task<string>> inCallBackStatusFunction = null, wbm_common.DataObjects.LongRunProcessHeader.dbRow longRunProcessHeader = null)
        {
            Result<List<wbm_common.DataObjects.Site.dbRow>> rc;
            List<wbm_common.DataObjects.Site.dbRow> Sites;
            switch (_SiteSearchData.SearchMode)
            {
                case wbm_common.DataObjects.Site.Search.SearchModeType.Undefined:
                    {
                        _SiteSearchData.GenerateSqlQuery();
                        rc = await _WBMDB.Site_ListBySql(_SiteSearchData.SqlQuery, _SiteSearchData.ListSqlParameter);
                        break;
                    }
                case wbm_common.DataObjects.Site.Search.SearchModeType.SingleID:
                    {
                        rc = await _WBMDB.SiteList_SearchByID(_SiteSearchData.ID);
                        break;
                    }
                case wbm_common.DataObjects.Site.Search.SearchModeType.AllRecords:
                    {
                        rc = await _WBMDB.Site_List();
                        break;
                    }
                default:
                    {
                        logger.Error("SiteHelper.ExecuteSearch: Site Search Mode Type Error");
                        rc = Result.Exception<List<wbm_common.DataObjects.Site.dbRow>>("Internal Exception occured");
                        break;
                    }
            }
        ...
        {
            private wbm_common.DataObjects.Site saveObject { get; set; }
            private wbm_common.DataObjects.Site.dbRow saveRow { get; set; }

            private wbm_common.DataObjects.Site.dbRow existRow { get; set; }
            ...
            public SiteSave(wbm_common.DataObjects.Site saveObject, int UserID, WBMDatabase wBMDB)
            {
            ...
            public Boolean CheckExist()
            {
                ...
                if (saveRow.ID > 0)
                {
                    Result<wbm_common.DataObjects.Site.dbRow> rc = _WBMDB.Site_ReadByID(saveRow.ID).GetAwaiter().GetResult();
                ...
                {
                    Result<wbm_common.DataObjects.Site.dbRow> rcexist = _WBMDB.Site_SearchByID(saveRow.ID).GetAwaiter().GetResult();
                    ...
                    else
                    {
                        ...
                        logger.Error("SiteSave ID " + saveRow.ID.ToString() + " already exists");
                        ReturnState = 400;
                        SavedSuccessfully = false;
                        return false;
                    }
            ...
            public Boolean CheckForChanges()
            {
                ...
                return ct.CheckForChanges(existRow, saveRow, wbm_common.DataObjects.Site.ListChangeTrackerData);
            }
            ...
            public void SetChanged()
            {
                ...
                else
                {
                    ...
                    changeHeaderRow = WBM_API.Helpers.ChangeTracking.Create_ChangeHeader_dbRow((int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.Site,
            ...
            {
                Result<int> sr = _WBMDB.Site_Insert(saveRow).GetAwaiter().GetResult();
            ...
            public Boolean Save_Update_or_Delete()
            {
                ...
                Result<int> cuResult = _WBMDB.Site_Update(saveRow).GetAwaiter().GetResult();
                ...
                if (cuResult.IsSuccess)
                {
                    ...
                    if (chResult.IsSuccess)
                    {
                        ...
                        int ChangeHeaderTableID = (int)wbm_common.DataObjects.ChangeTracking.ChangeHeaderTableID.Site;
